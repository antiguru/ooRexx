...
//!
//! Native-or-Rexx and the token, keyed by (scope, name): present in that
...
//!
//! # The token is a citation, not a body identity
//!
//! Two tokens can name one C++ function -- `Set`'s `HasItem` is written
//! `IdentityTable::hasIndexRexx` and `IdentityTableClass.hpp` declares no
//! such member -- and one token can reach three behaviours chosen by the
//! contents class the receiver allocated. Count tokens for an upper bound on
//! bodies and never for a count of behaviours.
//!
//! Refresh with
...
// ---- Setup.cpp, for the native-or-Rexx column and the token ----

/// One native method the interpreter defines: its token and arity operand.
type Native = (String, String);

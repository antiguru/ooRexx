...
fn general_overflow_message_is_42_901_substituting_the_adjusted_exponent_and_the_constant_9() {
    ...
    assert!(matches!(
        err,
        ArithError::Overflow {
            adjusted_exponent: 1999999999
        }
    ));
    assert_eq!(err.additional(), vec!["1999999999", "9"]);
    assert_eq!(
        err.message(),
        "Arithmetic overflow; exponent (\"1999999999\") exceeds 9 digits."
    );
}
...
fn general_underflow_message_is_42_902_substituting_the_raw_exponent_not_the_adjusted_one() {
    ...
    assert_eq!(
        err.message(),
        "Arithmetic underflow; exponent (\"-1000000010\") exceeds 9 digits."
    );
}
...
fn power_magnitude_precheck_message_is_42_001() {
    ...
    assert_eq!(
        err.message(),
        "Arithmetic overflow detected at:  \"100**999999999\"."
    );
}
...
fn system_resources_message_is_the_bare_error_5_no_substitution() {
    ...
    let err = n("4.0")
        .div(&n("2"), 999_999_999_999_999_999, DivOp::Divide)
        .unwrap_err();
...
fn code_still_matches_every_variant_after_the_split() {
    assert_eq!(
        n("1").div(&n("0"), 9, DivOp::Divide).unwrap_err().code(),
        42
    );
    assert_eq!(
        n("9e999999999")

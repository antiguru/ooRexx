...
* **Phase status is a mutable aggregate, and it is the one that actually rots.** "Owned by 4c", "not yet implemented", "in scope since Task 3", "the ten prefixes this crate emits today" are all facts about where the implemented/not-implemented boundary sits, and that boundary moves every time a task lands. Measured at the 4b gate: **522 comment lines** named a phase or used a boundary-time word, and they produced essentially every correction commit in the phase -- while the 796-row `corpus/keyword-exempt.txt`, which *derives* the same facts from the loud message and polices them in both directions, needed none across thirteen tasks. Same data, different medium, opposite outcome. **Assert the boundary or do not write it down.** Where a property holds regardless of where the boundary sits, state the property: `tests/support/mod.rs`'s normalisation covers every marker `trace_prefix_table` lists, whether or not this crate emits it yet, and that sentence cannot go stale.

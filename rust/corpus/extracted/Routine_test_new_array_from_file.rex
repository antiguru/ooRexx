/* extracted from Routine::test_new_array_from_file */
::routine main public
  ...
  do counter i fn over files     -- test source and encoded compiled
      -- read content of file into an array
     s=.stream~new(fn)~~open("read")
     arr=s~arrayin
     s~close
     if i=3 then self~expectSyntax(13.1) -- Error 13.1:  Incorrect character in program "@" ('40'X).
     r=.routine~new("r", arr)    -- create routine from array
     self~assertEquals(43,r[])
     self~assertEquals(47,r[1,'a',3,4])
  end

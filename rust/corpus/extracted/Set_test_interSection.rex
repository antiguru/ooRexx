/* extracted from Set::test_interSection */
::routine main public
  ...
  self~assertTrue(sameContent(d1, d2), "subtest8: 'other' is an 'OrderedCollection'")
   ...
   -- A simplistic intersection test using a table
  ...
  expectedSet = .set~of("Tom", "Frank")

  resultObj = receiverSet~interSection(argumentTable)
  self~assertTrue(resultObj~isA(.set), "Result object after intersection must be same class as receiver")
  self~assertTrue(sameContent(resultObj, expectedSet), "Result after intersection should match expected")

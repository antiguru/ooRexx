/* Table C method rows: InvertingComparator, instance arm -- one line per method
   corpus/docs/class-methods.txt documents on this arm, asked of the
   instance `.InvertingComparator~new(.Comparator~new)` answers, in the row set's own order. That
...
o = .InvertingComparator~new(.Comparator~new)

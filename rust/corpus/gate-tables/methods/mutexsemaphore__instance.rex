/* Table C method rows: MutexSemaphore, instance arm -- one line per method
   corpus/docs/class-methods.txt documents on this arm, asked of the
   instance `.MutexSemaphore~new` answers, in the row set's own order. That
...
o = .MutexSemaphore~new
say 'instance' o~hasMethod("acquire")

/* Table C method rows: StreamSupplier, instance arm -- one line per method
   ...
   instance `.Stream~new(filespec('path', .context~package~name) || '../fixtures/streamsupplier_seed.txt')~supplier` answers, in the row set's own order. That
...
o = .Stream~new(filespec('path', .context~package~name) || '../fixtures/streamsupplier_seed.txt')~supplier

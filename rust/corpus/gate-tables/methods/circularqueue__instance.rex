/* Table C method rows: CircularQueue, instance arm -- one line per method
   corpus/docs/class-methods.txt documents on this arm, asked of the
   instance `.CircularQueue~new(5)` answers, in the row set's own order. That
...
say 'instance' o~hasMethod("resize")

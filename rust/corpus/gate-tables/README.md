...
* `fixtures/` -- files a `class-set.txt` construction expression reads.
  ...
  seekable file; the expression locates it as `filespec('path',
  .context~package~name) || '../fixtures/streamsupplier_seed.txt'`, relative
  to the running probe's own path rather than an absolute path baked in at
  extraction time, so it resolves wherever the repository is checked out.

/* PARSE's sources, its two case options, and the trace shape a target's own
   ...
   against an upper-case `X` in the source and still assigns the source's own
   bytes on either side of it, so it prints `[Mi][eD case]` -- an engine that
   folds the source in order to compare prints `[MI][ED CASE]` instead, which
   is what C4 prints from UPPER doing it on purpose. C4/C5 also pin that
   `upper caseless` and `caseless upper` are order-independent, and C6 is
   LOWER on its own, with no pattern for a fold to hide in.

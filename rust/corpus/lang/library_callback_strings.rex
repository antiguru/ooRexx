/* The thread table's string, buffer and mutable buffer members through
   orxmethod's methods: a string value as an object and as a C string, a
   string's bytes from an offset, its case, whether an object is a String,
   a Buffer as a method's CSELF, and a MutableBuffer's length, capacity and
   bytes written through the address the capacity call answers. */
t = .T~new
...
say t~is('abc') t~is(12) t~is('12') t~is(1+1) t~is(1/3) t~is(2**70) t~is(.array~new) -
  t~is(.nil) t~is(.string~new('x')) t~is(3.5) t~is(10000000000)
m = t~nmb(10)
...
say 'grow' t~smbc(t~nmb(10), 15) t~smbc(t~nmb(4), 5) t~smbc(t~nmb(4), 9)
b = .BufferTester~new(42)
say b~value b~value2 t~isb(b) t~isb('x')
::class T

 ...
 *
 * They are different answers a program can tell apart, which is what byname,
 * byany and bysyntax are for. A trapped NOMETHOD reports CONDITION('C') as
 ...
 * it.
 *
 * Whose trap table decides is the running activation's, and outerwins,
 * innersyntax and methodmiss are the answers that follow from it. An internal
 * CALL inherits its caller's traps, so outerwins' NOMETHOD handler takes a
 * miss raised inside innerraise even though innerraise armed a SYNTAX handler
 * of its own, and innersyntax calls the same routine with no NOMETHOD armed
 * anywhere and gets that SYNTAX handler instead. A method activation inherits
 * no traps at all, so methodmiss arms NOMETHOD and SYNTAX alike and gets the
 * SYNTAX handler: the NOMETHOD condition is never raised there, because
 * nothing the running activation can see would take it. A build that asked
 * the whole activation stack instead answers methodmiss with the NOMETHOD
 * handler.
 *
 * Measured, rc 0.
 */

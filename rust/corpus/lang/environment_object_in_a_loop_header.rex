 ...
 *
 * What is here is the boundary either side of it. FOR and a bare DO's repeat
 * count read the value's TEXT through whole_nonneg rather than converting it
 * to a number; NUMERIC DIGITS renders the value and parses the bytes in
 * set_digits_str, which never asks for a number either. So both
 * implementations answer 26 from the same rendering, and a builtin argument
 * does the same and answers 40. Making the whole header loud would take every
 * line below from an agreement to a refusal.
 *

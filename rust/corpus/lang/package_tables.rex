/* Package's ten table readers, on a package carrying more than one of every
   ...
   This program's stdout is compared BYTE FOR BYTE. The tables it prints are
   hash-ordered on both sides, and the two orders differ in general -- but the
   names here are short enough that this crate's sorted walk and the oracle's
   bucket walk happen to agree, so it is not on corpus.rs's
   HASH_ORDERED_STDOUT and Deviation 8 does not cover it. `package_writes.rex`
   is the one program that needs the licence. **The agreement is a
   coincidence of these names, not a property**: restoring long names, which
   is what the defect below asks for once it is fixed, will need this program

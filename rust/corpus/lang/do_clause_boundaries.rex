/* Where a plain DO offers a queued condition its boundary, on stdout.
   ...
   The oracle's rule, which every block below reads back: one delivery per
   clause boundary, walking forward, and a plain DO is TWO clauses -- the
   header, which RexxInstructionSimpleDo::execute ends as soon as it has
   opened the block, and END, which is an instruction of its own.

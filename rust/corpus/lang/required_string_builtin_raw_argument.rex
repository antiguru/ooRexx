 ...
 * string.
 *
 * The last rows are the same position with the protocol armed by a NOSTRING
 * trap instead of by a makeString, which is the arming route that needs no
 * directive at all -- a converted position raises there and this one does not.
 * They read the stored value with ~isA so that nothing on those lines renders
 * an object and raises for an unrelated reason.
 *
 * Phase 5a Task 14, fix round 1.
 */

 ...
 * rather than only a final count:
 *
 *   'S'          the starting segment.
 *   '[H4]'       the first handler firing, SIGL 4 -- `raiser`'s own RAISE
 *                line, not the calling clause's.
 ...
 *   '[H10]'      the *second* handler firing, from the *same* `call on`
 *                with no intervening re-arm -- this is the segment that
 *                does not exist if CALL ON traps disarm like SIGNAL ON's.
 ...
 * the handler call) skipped, ZLOG reads `S[H4]1:VK2:V` -- the '[H10]'

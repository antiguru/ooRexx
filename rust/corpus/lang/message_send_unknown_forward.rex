 ...
 *
 * Where the forward's result goes decides the exit status, so the arms are
 * written differently on purpose. Class K's UNKNOWN returns, and its rows are
 * expression positions; class J's only says, and its rows are whole clauses --
 * `say .j~zork(1,2)` over a method that returns nothing is Error 91.999 at
 * rc 165, because SAY demands a result.
 *
 ...
 * name argument allocates and the array argument, built before it, has to
 * survive that. That is what makes this program the witness for the arguments
 * array's rooting: with collect_stress.rs's collect-on-every-allocation and
 * the array's root removed, this row's a~items sends to a collected array and
 * is refused at rc 120, where a message name of seven bytes or fewer leaves
 * the whole subset green.
 *
 * Measured, rc 0.
 */

/* A ::ATTRIBUTE SET written in Rexx is a method body like any other, and it
   ...
   The `a` pair is the untraced half. S4 is the setter's own line, so an
   engine that resolved the assignment to the getter's entry, or dropped the
   `=`, prints S3 twice and S4 never -- and one that ran no body at all is
   loud, since nothing else answers either name.

   The `b` pair is why this program is in RAW_STDERR_COMPARISON. The >I> and
   <I< lines carry the RESOLVED name, and `"B"` from the getter against `"B="`
   from the setter is the only place a program can see that the two entries
   are distinct; an engine announcing the directive's own spelling prints
   `"B"` for both, on stderr alone, at rc 0. The bodies turn TRACE I on
   themselves rather than inheriting it, because trace does not cross into a
   method activation -- measured, a traced sending clause echoes its own
   >E>/>L>/>A> lines and no >I> at all.

/* The thread table's package, library and executable members through
   orxmethod's methods: a package loaded by name and from source text, its
   prologue run once, its routines, classes and methods, a routine and a
   method built from text, a routine called with an array of arguments,
   and a program called by name. */
t = .T~new
p = t~loadpkg('pk1.cls')
...
say t~callprogram('prog.rex', .array~of('a', 'b'))
exit
count: procedure
  n = 0

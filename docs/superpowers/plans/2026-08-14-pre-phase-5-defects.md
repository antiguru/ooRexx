...
- [x] **Step 1: the reset's own rule, established.** Swept `numeric digits`, `numeric fuzz` and `numeric form` with no operand from starting `DIGITS` 9, 12 and 30, `FUZZ` 0, 5, 8, 11 and 20, and `FORM` both ways -- every combination where the starting state is reachable, 72 programs. **Each no-operand form writes its own setting to the language default and leaves the other two untouched**, at rc 0 with no exception anywhere: `numeric digits` stores 9, `numeric fuzz` stores 0, `numeric form` stores `SCIENTIFIC`. The default is the language's and not the package's -- with `::options digits 15` in force, `numeric digits` still stores 9; with `::options digits 12 fuzz 3`, `numeric fuzz` still stores 0.

  **The ANSI draft says the opposite, and citing it is how this stops looking like an oversight.** `X3J18-199X` 8.3.15.1 gives the no-operand form `Value = 9` and then runs the same `if Value<=#Fuzz.#Level then #Raise 'SYNTAX',33.1` the operand form runs -- which is what this crate did. So the interpreter diverges from the standard here, and this crate follows the interpreter. (The draft is at `https://www.rexxla.org/rexxlang/standards/j18pub.pdf`; it carries the whole standard including its executable Rexx model, not just the front matter.)

- [x] **Step 2: what `fuzz >= digits` means, established -- and it is two rules, not one.**

  **`fuzz == digits` is ordinary.** The comparison runs at zero digits: at `DIGITS 9 FUZZ 9`, `1 = 1.0`, `1 = 1.000001` and `1e5 = 1e5` are all `1`.

  **`fuzz > digits` is degenerate, and it is not "compare at zero digits" and not "ignore FUZZ".** `NumberString::comp` (`NumberStringClass.cpp:3190`) computes `digits -= fuzz` into a signed `wholenumber_t`; every length test below it then compares a non-negative length against a negative bound, so the digit-array shortcut is skipped, and `addSub` (`NumberStringMath.cpp:574`) truncates both operands to `digits + 1` and finds the left one large enough that the right cannot affect the result, handing the left operand straight back. **Its sign -- shared by both operands at that point -- is the whole answer.** Measured at `DIGITS 9 FUZZ 20`, all rc 0: `1 = 1.0` and `1e5 = 1e5` are `0`, `1 > 1.0000000001` is `1`, `1 < 1.0000000001` is `0`, `-1 < -1.0` is `1`. Opposite signs and zero are decided before the precision is reached and answer the same at any FUZZ. Arithmetic, `DATATYPE('NUM')` and the strict operators are untouched -- none reads FUZZ.

- [x] **Step 2b (found by Step 2's own probe, and not in this task's first draft): a comparison of two integer objects inside `DIGITS` never reads `FUZZ` at all, and this crate applied it. Reachable today at any non-zero `FUZZ`, with no reset involved.** `RexxInteger::comp` (`interpreter/classes/IntegerClass.cpp:1191`) subtracts two integer objects directly when `Numerics::isValid` holds for both, and only anything else falls to the fuzzed `NumberString::comp`. Measured at `DIGITS 9 FUZZ 8`, `100000000 = 100000001` is `0` where this crate answered `1`; so is the same pair reached through `trunc`, `x2d` or `c2d`, and so is a controlled loop's bound test -- `do zi = 100000002 to 100000001` runs no passes. The adjacent rows that stay fuzzy are the ones where an operand is not an integer object: `100000000.0`, a value a digit wider than `DIGITS`, and `format(100000000)`.

  **The interpreter's test is which class built the value; this crate's `SmallInt` tag is admitted on what a value renders as (D15), and the two do not coincide everywhere.** They agree across every spelling probed -- literals quoted and bare, variables, arithmetic results, `%`, `/`, `value()`, `abs`, `sign`, `max`, `min`, `format`, `length`, `substr`, `strip`, `copies`, concatenation -- once `trunc`, `x2d` and `c2d` tag their integer results and a controlled loop carries its own value's representation across the increment rather than re-reading the tag. **That list is a probe's sample and not an enumeration**, so a builtin outside it that the interpreter builds an integer for is still open; the method for extending it is the `s7`/`s10` shape -- compare the builtin's result against an adjacent integer literal at `DIGITS 9 FUZZ 8` and see which path the oracle took.

- [x] **Step 3: done.** `Settings::reset_digits` and `reset_fuzz` replace the fallible value-form setters, which had no other caller; `numeric_order` branches on `fuzz > digits` before computing a working precision; `small_int_compare` and the loop bound drop their `fuzz == 0` guards; `Interp::integer_text` tags the builtin results the interpreter builds integers for. Covered by `ir_dual_cases/numeric-fuzz` on both engines with oracle-measured bytes, by `Settings`' own reset tests, and by `compare.rs`'s `fuzz_above_digits_orders_by_the_shared_sign`.

  **Two tests asserted the defect and were corrected rather than deleted.** `run/tests.rs`'s reset test said `numeric digits 20; numeric fuzz 15; numeric digits` raises 33.1 and its comment claimed the oracle was measured; the oracle prints `9 15` at rc 0. `ir_dual_cases/operators` expected `1000000 = 1000001` to be `1` at `FUZZ 3` and said so in prose; the oracle prints `0`, because both operands are integer objects. Both files were recording this crate's own answer, and `operators` carries a header saying every byte in it was measured against the oracle -- so the rule was in place and the row went in anyway.

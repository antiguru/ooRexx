...
- [ ] **Step 0b: Give it a negative control before trusting it.** Build a deliberately slowed arm -- a `black_box` spin in the region loop sized to a few per cent -- and confirm the harness reports a regression of about the size introduced. **This is not optional and it is what condemned the previous instrument:** 4b-M's control read +4.3%, +2.5% and +1.8% where it should have read within half a per cent, and a 7% signal sitting above a 6% artifact is not a measurement. A harness whose control passes silently is indistinguishable from one that never ran.
  ...
  **Size the control against the floor Task 8 measured, which is the tightest bound this phase has on instructions.** A control build differing only in `eval_node`'s three arms read **+0.74% on `emptyloop`'s IR arm** -- an axis with **no promoted read**, so the change could not have reached it. That is larger than four of the six per-axis moves in Task 8's own table, and its mechanism is not established. **The code-placement artifact is instrument-specific, and the two instruments need separate bounds.** Task 7-M2's negative control -- a driver-only change on an axis whose body reaches no promoted clause -- reads a cross-build ratio of **1.000000** on `instructions:u` (84 instructions apart in 4.04e10) and **1.02** to **1.03** on `cycles:u`, with the executed work held constant. So an `instructions:u` figure from `rexx-arms` resolves far below 0.75% and a per-axis instruction move of a tenth of a per cent is a reading rather than an artifact; a **cross-build** `cycles:u` figure below about 3% is not. Task 8's `+0.74%` was an instruction figure across a different binary pair and is not reproduced or refuted here; nothing licenses carrying it as a general floor, and the within-binary arm ratio is unaffected by it either way, since both arms share a layout.

*The paragraph this replaces read "a per-axis move under about 0.75% is therefore not yet distinguishable from this effect", written by me from Task 8's single figure. It attributed a bound to the instrument that belongs to one instrument on one comparison direction, which is the same shape as the two attribution errors this phase has already corrected.*
...
> **Correction, 2026-08-15: the reason clause is no longer this crate's mechanism, and the conclusion it supports still holds.** `56d9d1c86` replaced the single pending-trap slot with a queue that a boundary *drains*, so a boundary no longer delivers at most one. What it drains is bounded to the entries that were queued when it began, so a trap a handler queues while it runs still lands beyond the prefix that boundary owes, and the last member boundary is still not the last boundary with work. The sentence was accurate at `7a7f58491` (2026-08-11), the commit that added this paragraph, where `Interp` holds `pending_trap: Option<PendingTrap>`; read the mechanism from `Interp::pending_traps`' own doc comment rather than from here.
...
> **Both prediction paragraphs below are withdrawn** -- see "Task 9's recorded prediction" at the end of this section, written by the task after building it. Both were derived from prototype P1, which changed the small-integer path and not operand evaluation, and this task changes the second and not the first. They are kept rather than deleted because the withdrawal argues against them by name.

**~~Predicted movement: `compound` only.~~ WITHDRAWN 2026-08-11 -- see "Task 9's recorded prediction" below, which replaces this heading and the paragraph under it.** Both were derived from 4d-1's prototype P1, and **P1 is not the change this task makes**: P1 extended the small-integer path to more *operators*, while this task changes how the *operands* are evaluated and leaves `small_int_arith` covering `+`, `-` and `*` exactly as before. The two share no mechanism, so P1's per-axis figures predict nothing here in either direction.

**The withdrawn text is kept below rather than deleted, because the way it was wrong is the phase's most repeated error and the third instance was mine.**

> *~~`arith` is predicted not to move, and an earlier wording of this line predicting both axes was wrong.~~ P1 moved `arith` -0.7%, inside noise, and `phase-4d-attribution.md:232`-`:235` gives the reason and calls it a prediction rather than a miss: `arith`'s divisions have non-integer operands, so a small-integer path cannot reach them. Its 28.7% share "is therefore real and not addressable by this fix". ~~An `arith` that does not move is this task succeeding.~~*

**Every sentence quoted above is true about P1 and false about this task**, and the last one inverts outright. It is a statement about which *operation* runs; this task changed how the *operands* are produced, and `i / 3` has exactly as much operand evaluation as `t.k + 1` does. **An `arith` that does not move is now the surprise rather than the success.**

**What went wrong in the correction that produced it, stated so it is not repeated.** I wrote that paragraph while fixing an earlier version that predicted both axes, and I checked the citation carefully -- `-51.6%` really is P1 and really is on `compound`. What I did not check is whether P1 was the same *change* as this task. Verifying that a figure is correctly attributed to its own prototype says nothing about whether that prototype is the thing being predicted, and a better-sourced citation for the wrong referent reads more convincing than the vague version it replaced.
...
#### Task 9's recorded prediction, written before anything was measured (2026-08-11)

**The "predicted movement: `compound` only" heading above is withdrawn, and so is the reasoning under it.** Both were derived entirely from 4d-1's prototype P1, and **P1 is not the change this task makes.** P1 extended the *small-integer path* to `/`, `%`, `//` and `**`, leaving operand evaluation alone; this task extends *neither* -- `small_int_arith` still covers `+`, `-` and `*` only -- and instead removes the `eval.rs` recursion that produced the operands. The two changes share no mechanism, so P1's per-axis figures predict nothing here, in either direction. In particular `compound`'s -51.6% came from `k = i // tails` leaving the `Number` path, and `//` still takes that path after this task.

**What this task actually changed**, and therefore what each prediction rests on: an arithmetic expression whose operands are all literals, constant symbols or bare symbol reads now compiles to native ops for the whole tree, so what goes away per operator is the `eval`/`eval_node` recursion for both operands -- the depth bookkeeping, the post-order trace hook, the node dispatch -- plus the root frame and two temp pushes `eval_arithmetic` made. What stays is the arithmetic itself, unchanged and shared.

* **`varlookup` should move, and the plan predicted it would not.** `x = x + 1` is exactly the promoted shape, it is half the loop body, and this is the axis where the removed per-operand overhead is the largest fraction of what a clause does. If any axis moves most, this is the one.
* **`compound` should move, by less than 4d-1's -51.6%** and by a different mechanism: `t.k = t.k + 1` and `k = i // tails` both promote their operands, but the `//` itself is untouched, and a compound read is expensive enough that the recursion removed around it is a smaller share.
* **`arith` should move too, and the plan's "an `arith` that does not move is this task succeeding" is wrong for this task.** That sentence is sound about P1 -- a small-integer path cannot reach `i / 3`, whose operands are not both integers -- but it is a statement about the *operation*, and this task changed the *operands*. `a = i / 3`, `b = a * a - 1` and `total = total + b + d` all have fully promotable operand trees, so every one of them loses the same recursion whether or not the division is ever small-integer. **An `arith` that does not move is the surprise here, not the success.**
* **`emptyloop`, `startup` and `alloc4c` should not move**, having no arithmetic in a measured loop; `strings` likewise.
* **The patch table should be at or below noise on every axis, and a win that survives `QUICKENING = false` is expected rather than alarming.** This is the concern worth recording before the measurement: the hint's schema forbids it from removing a precondition, and for small-integer arithmetic the precondition *is* the specialisation -- decode both operands, check both against `DIGITS`, apply the operator. There is nothing left for a hint to skip except that same check. What it can do is skip a *hopeless* one, which is the direction implemented: a site that has fallen through once goes straight to the general path. So the table should pay on `arith`, whose divisions never take the fast path, and cost a relaxed load on `compound` and `varlookup`, whose sites always do. **Exit criterion 5 is the right instrument and its likely answer is that the win survives the deletion.** That does not condemn D22's schema; it says small-integer arithmetic is a poor consumer for it, which is a finding Task 11 should record either way.
* **The `AtomicU32` price should be zero or near it** on this workload: the load is relaxed, from a line touched by nothing else, and the branch on it is perfectly predicted after the first execution.

**One correction to D22's stated justification, found by building it.** The rule's reason -- "`Cell<Op>` is not `Sync`" -- is a forward-compatibility argument, and the plan already notes it does not bite today. What does bite today is plainer and was not stated: a chunk is reached as `&Chunk`, so a site cannot record anything at all without interior mutability. `Cell<u32>` would serve for that, and the choice between the two is the only part that is a bet. The type is shaped so Step 3c is a three-line edit in one place.

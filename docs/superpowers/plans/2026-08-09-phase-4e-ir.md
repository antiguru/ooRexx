...
The four `SELECT` shapes it must keep are already in `BRANCH_CASES` (`tests/ir_dual.rs`), added by
Task 4b before anything was promoted, so its regression net is in the tree before it starts.

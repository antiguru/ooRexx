...
**A surface the plan did not ask for.** `rexx-run` now reads `REXX_ENGINE`, because no task in this phase could otherwise measure the IR arm at all. The plan's rule that "engine selection is not an environment variable" is about the *carrier into the interpreter*, which is still `Invocation`; the library `rexx-exec/src` outside `src/bin/` still contains no `env::var`. The plan's flat claim that "`rexx-exec/src` contains no `env::var` at all" is now false by path and is corrected here rather than left to rot.

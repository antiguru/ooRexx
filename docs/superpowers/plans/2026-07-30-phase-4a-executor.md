...
**Task 3 measured these and the answer is not the one this plan first recorded, so inherit the corrected numbers.** The interpreter thread is **512 MiB**. `eval` costs 784 bytes per level in debug, but **`eval` is not what binds the stack**: the phases are sequential and each unwinds before the next, and the compiler-generated `Drop` of the parsed tree costs **~820 bytes per level**, more than `eval` does. The honest budget is therefore **~860 bytes per level and ~620,000 levels in debug**, not the ~670,000 that sizing against 784 alone suggests.

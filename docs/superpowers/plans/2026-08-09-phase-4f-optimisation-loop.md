...
* **Whether `Op` should be 16 bytes rather than 12 (raised 2026-08-11 by Moritz).** `size_of::<Op>()` is 12 and pinned by `const _: () = assert!(size_of::<Op>() == 12);` in `ir/mod.rs`, so widening it is a deliberate act rather than an accident -- which is what the assertion is for. **12 does not divide 64**, so ops straddle cache lines and an op's line membership depends on its index; 16 would put exactly four per line, none straddling, and turn the stream's index arithmetic into a shift.

  **The trade runs both ways and neither side has been measured.** Against widening: a 64-byte line holds 5.33 ops at 12 bytes and 4 at 16, so a **sequentially walked** stream -- which is what `run_ops` does -- touches 33% more lines for the same op count, and density usually wins for sequential access. For widening: no straddling op, cheaper addressing, and a jump landing mid-line is currently possible.

  **Phase 4e's item 3 did not answer this and must not be read as if it had.** It ruled out "the op stream's stride" as an *explanation for the phase's +21 per pass on `emptyloop`*, and it ruled it out on the ground that `size_of::<Op>()` **never moved during Phase 4e** -- a statement about what changed, not about what the right width is. The question here is untested.

  **It is a microarchitectural question and it belongs in this loop rather than before it**, which is why it is recorded here rather than chased in the 4e handoff. It is also a good early candidate: it is one constant, one assertion and a rebuild, and `emptyloop`'s own measured codegen sensitivity means the axis most likely to show it is also the axis least able to prove it -- so measure it on `varlookup` and `arith` first.

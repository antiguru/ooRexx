...
**(a) Resolve to the definition line of the named function or the named declaration.**
...
| `:124` | `inheritInstanceMethods` — donation with no superclass edge | **nothing found, and here is how wide the search was.** `/bin/grep -aril --include='*.testGroup' 'inheritInstanceMethods' ootest/` exits **1** — no file tree-wide. Searching for the *effect* rather than the name, `/bin/grep -ariEn --include='*.testGroup' '(supplier\|\.set\|\.bag\|\.relation)[^;]*~hasMethod\|hasMethod\("(ALLITEMS\|ALLINDEXES\|SUPPLIER)"' ootest/` also exits 1. Consistent with the mechanism being image-build-only and removed by `Setup.cpp:1809` |
...
| `:136` | install is three passes; a cycle is 98.911 | **both, and the first draft of this row said the passes are pinned by nothing because it searched for `ACTIVATE` as a name rather than for a pass-ordering assertion.** The **cycle**: `base/directives/CLASS.testGroup`, pattern `98\.911`, 5. The **three passes**, by two tests in the two groups that draft dismissed: `base/directives/REQUIRES.testGroup:320` `test_activate` writes a requires file (`:324`) whose `::class test public` carries `::method activate class` doing `.local~activatetest = .test2`, with `::class test2 public` declared **after** it in the same file, and asserts at `:331` that `.local~activateTest` is that class object — so every class must be created before any `activate` runs, **pass 3 after pass 1**, and a create-then-activate-each implementation fails it. And `base/directives/CONSTANT.testGroup:496` `test_expression_activate`, commented "expression constants are evaluated at class creation time / they should already be availabe to the class activate() method", runs the `::resource activate` at `:486`-`:494` whose `::method activate class` (`:489`) reads `::constant d (2 * 2)` (`:493`) and asserts `4` at `:499` — **pass 2 before pass 3**. Together they pin the structure `PackageClass::processInstall`'s three loops (`:1276`, `:1285`, `:1294`) implement, which is the same row the ledger's own `processInstall` correction is about |
| `:137` | class-object initialization: `INIT`, then `INHERIT`, then `ACTIVATE` | **partly, through `:136`'s pair; the `INIT`-before-`INHERIT` half is what nothing found reaches.** `REQUIRES.testGroup:320` and `CONSTANT.testGroup:496` above pin `ACTIVATE` as last. For the other half, the searches run: no test group carries both an `::method init class` and an `::method activate` — measured by intersecting `/bin/grep -aril --include='*.testGroup' '::method[[:space:]]+.?init[[:space:]]+class'` with `/bin/grep -aqiE '::method[[:space:]]+.?activate'`, which is empty; and `/bin/grep -ariEn --include='*.testGroup' 'init[^;]*before[^;]*(inherit\|activate)\|activate[^;]*after' ootest/` returns nothing. So the discriminator the spec pins its 5a/5b boundary on — `self~hasMethod("MM")` answering 0 in `init` and 1 in `activate` — has no upstream counterpart under those searches |
...
| `:143` | `PROTECTED` routes the send through the security manager | `base/security.manager/SecurityManager.testGroup` — pattern `~setEntry\|~entry\(`, 32 |
...
| `:169` | `GUARDED`/`UNGUARDED`, `REPLY`, `GUARD` legality | `base/keyword/GUARD.testGroup` — pattern `guard (on\|off)`, 60; `base/keyword/REPLY.testGroup` — pattern `\breply\b`, 33 |
| `:170` | the `*-* Compiled method "X" with scope "Y".` traceback line | **nothing found in the object model.** `/bin/grep -ariEl --include='*.testGroup' 'Compiled method' ootest/` returns exactly one file, `base/source.file/incorrectCharacters.testGroup` (6 lines), which is about source encoding. No object-model group asserts the frame line |
...
The first draft of four such rows was wrong, every time for the same reason: **the search was
narrower than the sentence it licensed.** `:120` searched the hierarchy the file *declares* and
missed the one it *builds*, in the same file. `:136` searched for `ACTIVATE` as a name and missed two
tests that assert pass ordering without using the word as a claim. `:141` looked for a group *about*
SELF and SUPER and missed two that assert their values. `:153` searched for the order by name and
missed a test that asserts one of its step boundaries. **A reader who doubts a negative here should
re-run its stated patterns first and then widen them, because widening is what corrected all four.**
...
awk 'tolower($0) ~ /^[[:space:]]*::class/ { cls=$0 }
     tolower($0) ~ /external[[:space:]]+["'"'"']library[[:space:]]+rexx/ { print FILENAME":"NR" <= "cls }' <files>
```

— **case-insensitive and accepting either quote**, then each owning class's `init` traced to see
whether it reaches one. The quoting matters and an earlier draft of this paragraph said
`EXTERNAL 'LIBRARY REXX …'` with a single quote only. `StreamClasses.orx` carries three
double-quoted ones — `:510` `external "library REXX"`, and `:546` / `:547`
`external "LIBRARY REXX file_separator"` / `"…file_path_separator"`, which are **precisely the two
D37 implements**. All three are `File`'s and `File` is on the list below for `file_qualify` anyway,
so no member is missed and the conclusion is unchanged; but a later task re-running the
single-quoted pattern gets a different denominator than the one that produced this table.

...
1. **`DO OVER` a `StringTable`** -- `rexx-exec: one of the interpreter's own objects as a DO header's
   ...
   `StreamClasses.orx:45`, and there is no route around it: the prologue's first loop is
   `do name over publicClasses`. **The mechanism behind it is hash-collection iteration order, and
   nothing in this plan assigns it.** The oracle's order is a fixed order and not a race -- twenty
   runs of one seven-class probe give one sha256 -- and it is the bucket order of
   `HashContents`: `hash = 31*h + byte` over the key (`StringClass.hpp:328`), bucket
   `hash % bucketSize`, `MinimumBucketSize` and `DefaultTableSize` both 17
   (`support/HashCollection.hpp:126` and `:130`), overflow appended from a free chain starting at
   `bucketSize`, iteration bucket-ascending then along each chain (`HashContents.cpp:489`).
   Modelled against that, classes `AA BB CC DD EE ZZ MM` iterate `DD CC BB AA ZZ EE MM`, which is
   what the oracle prints, and 17 is the only bucket size in `2..4000` that reproduces it.
   **Reproducing it in general also needs the crate's own insertion sequence to match the oracle's
   for the table being iterated**, which is
   true for a package's public classes (declaration order) and false for `.environment`, which this
   crate models as a subset rather than building the way `Setup.cpp` does.
   `Interp::public_classes_table` fills its table in sorted name order today and its doc says the
   order "is not observable" -- **that sentence is true only while this refusal stands.**

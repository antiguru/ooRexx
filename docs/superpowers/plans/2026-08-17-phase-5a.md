...
* **old Task 5** -- `ExprKind::Message` and `InstructionKind::Message` against native methods, on both
  engines; `resolve`/`invoke` split; `Interp::resolve`'s `start_scope` parameter, **and not the
  scope-override send (`~m:super`) that fills it**, which is Task 23's -- lines 150 and 1699 below
  say so and are right, and this line claimed the delivery until Task 23 measured it at `5b054d4b5`
  as rc 120 on both engines; dispatch chokepoint one of two; the `>M>` trace prefix; the `*-*
  Compiled method "X" with scope "Y".` frame for an **explicit** send.
...
| the spec's `INIT`/`ACTIVATE` discriminator with the superclass-chaining line **removed** | oracle rc 0, `K init, hasMethod MM = 0` / `K activate, hasMethod MM = 1` / `prologue` -- **byte for byte the spec's transcript**. `FORWARD` is 5b's, so the spec's own program is unreachable in 5a. **`self~init:super` was not a usable substitute either when Task 18 ran**: measured there, a message scope override was refused rc 120 on both engines, and the old plan's Task 5 landed `Interp::resolve`'s `start_scope` parameter, not the expression that reaches it; **Task 23 built that expression**, and this row's own decision does not turn on it. K's superclass is `.Object`, whose class `INIT` is a no-op, so the oracle's transcript is byte-identical with the line simply dropped, and that is what makes Task 18's verification runnable |
...
| `say .k~zork(1,2)` with `::METHOD unknown CLASS` and `use arg n, a`, the method **returning** `'unknown:' n` | oracle rc 0, `unknown: ZORK`. The `UNKNOWN` name arm needs no collection method; the **args array** arm does, which is why Task 11 precedes Task 12. The return is load-bearing: with the same method `say`ing instead of returning, the oracle is **rc 165**, `Error 91.999: Message "ZORK" did not return a result.`, because `say` demands one |
...
| `.context~package~annotation("AUTHOR")`; `.routines["R"]~annotation("AUTHOR")`; `.K~method("A")~annotation("AUTHOR")` for an `::ATTRIBUTE` and `~method("C")` for a `::CONSTANT` | oracle rc 0 `moritz` on every one; crate rc 120 at `.context~package`. **The readings held; the ownership this row drew from them did not.** All six targets read back in 5a: Task 17 populated `.ROUTINES`, so the `ROUTINE` route resolves here, and `.K~package` is a second route to the package object that this row did not consider, so the `PACKAGE` readback needs no `RexxContext`. `.context~package` itself is still rc 120 and is still Task 21's -- see Task 20's report |
...
**Bound the crate side too, and bound its memory as well as its time.** Every wrapper in this
document caps the oracle -- `ulimit -v 1048576` and a `timeout` -- and until now nothing said to cap
`rexx-run`. A differential probe therefore ran one side under a 1 GiB address-space limit and the
other under none, which is the asymmetry that took a session down on 2026-08-23: a comparison script
bounded the crate with `timeout -s KILL 20` and no memory limit at all, two runs looped for the whole
20 seconds, and an unbounded allocating loop on this machine has 124 GB and 94 GB of swap to take.
**Which bound, and what each one's failure looks like, measured 2026-08-23 rather than reasoned
about.** The first draft of this bullet asserted that `timeout -s KILL`, `memcap` and a kernel OOM all
produce rc 137 and that the code was therefore ambiguous three ways. Running it says otherwise, and
the difference matters:

| bound | crate | oracle |
|---|---|---|
| `ulimit -v 1048576` | **rc 134**, `memory allocation of 268435456 bytes failed`, a Rust abort | **rc 251**, a graceful `Error 5 System resources exhausted` on the raising clause |
| `memcap 1G` | **rc 137**, cgroup OOM kill | -- |
| `timeout -s KILL` | **rc 137** | -- |

So **rc 137 has two producers, a time kill and a cgroup OOM kill, and an address-space breach produces
neither.** A script must still distinguish the two that share 137 -- print elapsed seconds and the
bound in force beside the code -- but the three-way claim was wrong.

**And the address-space bound is not symmetric, which is the part that changes the advice.** Under the
same `ulimit -v`, the oracle raises a clean Rexx error and this crate aborts, so the two sides fail
*differently* and a harness reading three descriptors sees a divergence the interpreters did not
cause. That divergence is licensed -- matching the oracle under memory pressure is explicitly
best-effort in this project and no ordinary path pays for it -- but a probe must not read it as a
finding. **Prefer `memcap`, which kills both sides the same way:**

```
( memcap 1G env REXX_ENGINE=<engine> timeout -s KILL 10 target/release/rexx-run /abs/path/FILE )
```

The standing point survives all of it: **a crate-side run gets a memory bound as well as a time
bound.** Every wrapper in this document caps the oracle and nothing said to cap `rexx-run`, so a
differential probe ran one side under a 1 GiB limit and the other under none, and **a run whose two
sides carry different limits is not a differential measurement.**
...
**The last two axes joined the list mid-phase, and writing them here is the point of this
paragraph.** `dispatchclass` was added at Task 13 to give the send path a guard axis of its own, and
`bench-rexxcps/rexxcps.rex` is added from Task 15. Neither is retroactive: `dispatchclass` has rows
from Task 13 onward and rexxcps from Task 15 onward, so neither can attribute drift to an earlier
task, and a first reading is a position rather than a delta. Adding an axis is not the event that
forces the baseline to restart -- that is a change of pin, and this one is unchanged -- so the file
keeps its rows and simply gains columns of coverage from the task that introduced each.

**The pin's compiler moved out from under it at Task 18, and the staleness test cannot see that.**
`PINNED.md` records `rexx-run-15a1ffa98` as built under `rustc 1.97.1`; from Task 18 the head arm
builds under `rustc 1.98.0 (88d9e12ae 2026-08-18)`, which arrived on its own because
`rust-toolchain.toml` pins the `stable` channel and not a version. The plan's staleness check is
phrased over commits beneath the phase and is blind to this axis, which is the same shape as the
defect that check was written for, one axis over.

**Ruling: the pin stays and the baseline does not restart.** The 1% line is asked of a task's own
contribution, and the contribution arm `base>changed` builds both sides under whatever compiler is
current, so the gate quantity is unaffected. What the compiler change confounds is the accumulated
`pinned>head` ratio, which is a position and not a gate. Rebuilding the pin would not reproduce its
recorded sha256 and would throw away the accumulated series to fix a figure that is already
advisory. **Accumulated readings from Task 18 onward therefore span a compiler change and must not
be attributed to any task's work**; the cells at or above 1% are the ones that stood before the
change and are identical to every digit across it, which is what makes that attribution checkable
rather than assumed.

**`dispatchclass` existed for two tasks without appearing in this document, which is the defect this
paragraph closes rather than an aside.** It was carried in Task 13's dispatch prose and nowhere else,
so a task that did not happen to inherit that prose would have run a narrower guard and reported
green over an axis it never measured. A guard axis that lives only in a dispatch is not a control,
because nothing outside the dispatch can tell it was skipped.

**rexxcps is measured on `instructions:u` here and its clauses-per-second figure is not a guard
quantity.** It has no `n = <integer>` line, so `Workload::classify` reads it as `Fixed`: one size, and
no per-pass figure at all, since `Sitting::per_pass` lives on `Scaled`. `rexx-arms` takes an `--axis`
holding a `/` as a path, so the program stays in `bench-rexxcps/` and needs no code change and no
entry in `bench-programs/`, whose contents `rexx-bench-suite` asserts in both directions.

**The command line above was run before this paragraph was written, because a guard command nobody has
executed reads exactly like one that passed.** The axis was smoke-tested with the pinned binary given
as *both* builds, into a scratch baseline: exit 0, sixteen rows, the `axis` column reading `rexxcps`
rather than the path, `size` reading `small` alone, the three scopes `absolute`, `arm_ratio` and
`across_builds`, and **no `per_pass` row**, as the `Fixed` classification requires. The harness also
refuses `--rounds 1` with *"1 rounds is not a median with a spread; 3 is the least this harness
reports"*, so three is the floor.

**That smoke test also measured the axis's own noise floor, and it is the best on the board -- within
one sitting.** Same binary on both sides, the `pinned>head` `instructions:u` ratio came out 1.000016 on
the IR arm and 0.999984 on the tree-walker, a floor of about two thousandths of a percent, against a
cps spread of 3.10% on the same program. It retires about 19.0 billion instructions on the IR arm and
24.0 billion on the tree-walker, several times the largest existing axis, which is why its resolution
is finer rather than coarser: a longer deterministic run makes the ratio sharper where a longer timed
run only blurs it.

**The qualification matters and it is not a detail: that floor is a within-sitting figure, and it was
measured in one sitting.** The rule further up this section already says per-pass figures are
comparable within a sitting and not across one, and Task 14's fix round put a number on what that
costs -- a **byte-identical** binary, reproduced independently from a fresh build at `127fb74b8`, read
`6417.398606` and `6404.359463` instructions per pass on `dispatchclass`/ir in two sittings, and the
same binary's **per-round span is 33.97 instructions per pass**. That span, not the 13.039 difference
between the two medians, is the resolution a task should plan against on that axis. Nothing here
establishes the equivalent span for rexxcps, so **the two-thousandths figure above is a floor on the
ratio within a sitting and is not a claim about two sittings.** A task comparing across sittings on
either axis is making a comparison neither number supports.

**Why the cps figure is excluded, measured rather than assumed.** Interleaved on 2026-08-22, three
rounds each at an 8 GiB cap, the oracle's own cps median moved 3.10% across the rounds, and a single
pass minutes earlier read 17,351,863 against a 16,983,508 median -- about 5% between two sittings.
The widest accumulated `instructions:u` drift this phase has produced is 1.009953. A quantity whose
run-to-run spread is several times the signal cannot answer the guard's question, so cps belongs to
the end-of-phase `rexx-bench-suite` snapshot, quoted with its interval and never differenced against
another section.
...
**Which quantity that 1% governs, established by reading the harness rather than the prose.** The
ratio the bench emits under `build = pinned>head` is head's absolute count over the pinned binary's:
**drift accumulated by the whole phase since the pin, not the task's own delta.** No code applies the
1% line anywhere; it is a rule in this document. And read as accumulated drift it has already been
crossed twice without stopping anything -- measured, widest `instructions:u` median per task:
Task 9 at 1.010616 (`strings`/`ir`/`small`), Task 11 at 1.013710 (`strings`/`tw`/`small`), Tasks 12
and 13 both at 1.009685. **So the 1% line is a question asked of a task's own contribution**, which is
the figure a single interleaved sitting yields, and the accumulated ratio is context rather than a
gate. A task at or above 1% on its own contribution says which it is; a task whose contribution is nil
against an accumulated ratio over 1% says that, and the earlier task that raised it owns the answer.
...
**The sitting requirement is a question, not a proof obligation: can this change affect what
executes?** Some changes plainly cannot -- a comment, a test, a corpus program, a document -- and those
run no sitting and say so in a sentence. That is the whole of it. **Do not answer it by comparing
binaries.** Task 8 was asked for two `sha256` sums and Task 10 for a section table, and the effort went
into ELF forensics for a question that needed a reading of the diff: `debug = true` makes whole-file
identity unattainable, so the naive form of that check can never pass, and the refined form -- `.text`
identity, with `objcopy --dump-section=` because `--only-section=` compares two empty files and passes
by not looking -- answers correctly a question nobody had to ask. **Reproducible Rust builds were never
the goal.** Where the answer is genuinely unclear, run the sitting; it is cheaper than the argument.
...
* `directive-options.txt` -- table D's row set: per `dire.xml` section, the union of `<option>` names
  ...
  **The parser side is wider than `SUBDIRECTIVE_*` and the committed header is the authority on how
  much wider.** Running the narrow rule produced wrong rows, twice, which is R32 arriving on
  schedule: `SCIENTIFIC`, `ENGINEERING`, `INHERIT` and `NOINHERIT` reach the parser as `SUBKEY_*`
  arms of a `switch` **nested inside** `case SUBDIRECTIVE_FORM:` / `case SUBDIRECTIVE_NUMERIC:`, so
  the narrow rule marks four documented, implemented keywords `cross-reference` -- and contradicts
  the spec's own reason for the `position` column, that *"the parser's nesting is what settles it"*.
  And `::RESOURCE`'s `END` is tested by `!=` outside any `switch`, so the narrow rule gives it **no
  row at all** though it is both documented and implemented. **`rust/corpus/docs/directive-options.txt`'s
  header states the rule actually used**, and its `evidence` column records each arm's family and
  form so the narrow set is recoverable by filtering. The spec still carries the narrow phrasing at
  its own `:857`; this plan does not edit the adopted spec, and a task reading that sentence should
  read the header beside the data instead.
...
**A concatenation cannot serve here, and a comparison can -- which corrects what this paragraph said
before Task 6's own re-review measured it.** The programs are still `**` and prefix `-`: `**` so the
frame's method name is read rather than assumed, and prefix `-`, a different dispatch path whose frame
name collides with infix minus. Measured, each is rc 215 on both engines and differs from the oracle
in exactly one line -- `       *-* Compiled method "**" with scope "String".` and
`       *-* Compiled method "-" with scope "String".`.

**The reason first given for excluding both was false for comparison, and the measurement behind it
was too narrow.** `say b. > 1` prints `1` and `say b. || 1` prints `B.1`, both at rc 0, and so do the
`Array`-operand forms -- but every one of those probes has a **non-numeric** operand, where a
comparison falls back to string comparison and cannot raise. Give it two numeric operands and it
converts to a number like any arithmetic operator: measured, `numeric digits 1` with
`s. = '9.9E999999999'` makes `say s. > 1` rc 214 on the oracle, opening
`       *-* Compiled method ">" with scope "String".`, and the same holds for `<`, `>=`, `<=`, `=` and
`\=`. Strict `==` and `>>` stay frameless.

**Concatenation is genuinely total**, and that half of the original reason survives: the same overflow
program with `||` is rc 0 on both sides. `say .array~of(1) + 1` also stands as first measured -- error
97, **with no frame line**, because no method was found and so there is no method activation to name.

So the operator families split by whether the operator needs a *number*, not by whether it is
arithmetic: arithmetic always, comparison when both operands are numeric, concatenation never.
...
* walking the chain instead of merging reddens the diamond -- **but not the obvious diamond, which
  was measured during Task 7 and does not catch it.** A chain walk lands on the same method the merge
  does, because `MethodDict::add_method` is scope-keyed and idempotent, so removing `cascade_build`'s
  `has_scope` guards changes nothing either. The control needs a second diamond where **only the
  second-listed mixin overrides**: there a chain-walk lookup answers the base's method where the
  oracle answers the overriding mixin's;
...
**Done when** the wiring rows this task's own protocol can carry read `agree`, the rows it cannot are
each attributed to the task that moves them, and **the two controls this task owns are recorded as
run**, which is the first point at which either can flip something green.

**The criterion said "the classes this crate registers", and that was unsatisfiable when it was
written.** Corrected after Task 9 measured it. Two separate gaps sit inside that phrase and only one of
them was known:

* the **known** one -- `Queue`, `Stem` and `VariableReference`, deferred in `native_classes.rs` for a
  mechanism no task owned when this plan was written. **Task 21 owns it now** and lifts them;
* the one the phrase hid -- **eleven registered classes whose wiring row needs the prologue's own
  `~inherit` calls**, which `native_classes.rs` assigned to **Task 13** before Task 9 started. Measured:
  each of the eleven differs on exactly one probe line, `superclasses`, by exactly the
  `CoreClasses.orx` `~inherit` targets -- `String` from `Comparable`, `Array` and `List` from
  `OrderedCollection`, six from `MapCollection`, `Set` and `Bag` from `SetCollection`, and `Message`
  from two notification mixins.

A further set of rows fails at `.NAME` for classes the prologue defines rather than the registry, which
is the same Task 13 boundary seen from the other side. **The number this task reports is moved again by
Task 13 and then by Task 21**, and a row still non-`agree` here is a regression only if it is not
attributable to one of those two. The controls:
...
* `resolve` gains the **caller's receiver** and the **caller's package** (D27's amendment, D53). Task 13
  is what uses them; landing the signature here keeps Tasks 12 and 13 from rewriting each other.
  **This said "the caller's scope" until Task 10 read the C++.** `RexxObject::checkPrivate`
  (`classes/ObjectClass.cpp:609`) takes the sender from `activation->getReceiver()` at `:616` and
  compares it to the receiver; the only scope it reads is the **resolved method's own**,
  `method->getScope()` at `:628`. So the caller contributes a receiver and the scope comes from the
  method, and a parameter named for the caller's scope would have had nothing to put in it.
...
* the sitting, which this task's text called its real acceptance -- **and which cannot see the subject.**
  Measured during Task 10: **no axis program in the guard sends a message or makes a `CALL`**, anchored
  on the parser's own two `ExprKind::Message` constructors rather than on a token search. So the axis
  sitting says *nothing added is on an executed path*, never *the path this task reshaped did not get
  slower*, and the guard is silent about exactly the code the task exists to change.

**So the send and call paths get their own probes, and Task 10's readings are their baseline.**
Interleaved, three builds alternated inside each round -- the phase pin, the previous task's head, and
the task's own -- this task measured **about 3 instructions per native send**, **about 12 per method
send**, and **+18 per internal `CALL`**, the last deterministic to five decimals with `head/pin` equal
to `head/prev`, so Tasks 1 to 9 added nothing to that path. **None of the three touches the
1.695467-per-pass axis budget**, because no axis reaches them -- which is the whole point.

**Attribution came with a control, not a guess:** a build whose `entered_receiver` body is replaced by
`None` measures **one instruction per call below** the previous head, so the 18 is neither the parameter
threading nor `CallContext` growing by an `Option`; it appears when the decision is computed rather than
folded to a constant. `#[inline(always)]` changes nothing. No mechanism is named beyond that, because
the instrument does not support one.

**The `ir` arm on the send probes does not reproduce to the precision the axis rows do** -- readings of
0.99800, 1.00575 and 1.00452 across three rounds where the tree-walker arm held 1.00112, 1.00111 and
1.00106. Any figure quoted from these probes carries its spread and its round count, and the `tw` arm is
the one to reason from.

**Tasks 12, 13 and 21 widen these same paths.** Each reports against these three figures when it touches
them, or states that it does not touch them. Eighteen instructions per `CALL` is invisible to the guard;
three tasks each adding as much would stay invisible.

**Done when** the three constraints have a failing-when-dropped test each, both tables' 5a row counts
are unchanged, the sitting is recorded with `instructions:u` per axis, and the send and call probes are
recorded with their medians, spreads and round counts. Sitting required, and at or above 1% this task
says whether it is the change or the layout with an interleaved control.
...
**Build.** `checkPrivate`'s limb keyed on the **caller's receiver**; `checkPackage`'s keyed on the
**caller's package**; `PROTECTED` routed through the security-manager seam, which is what makes the
seam's placement checkable rather than nominal.

**Where the two access checks go, corrected against the oracle during this task.** This said they
"sit inside the chokepoint, not beside it". They do not: `messageSend` refuses the **lookup** and falls
through to the receiver's own `UNKNOWN`, so they belong in `Interp::resolve`, and only `PROTECTED` sits
at the seam. Measured, both engines, and the second program is the one that settles it: `say .K~m` with
`m` a `PRIVATE` class method **and an `UNKNOWN` class method present** is oracle rc 0 `unknown: M`
rather than 97.2, and it stays rc 0 `unknown: M` when that `UNKNOWN` is itself declared `PRIVATE` --
**the `UNKNOWN` lookup is not access-checked.** A design that put the refusal at the chokepoint would
have to raise 97.2 there and could not reach either answer.
...
**What it changes about refusals, and the instrument -- both halves of this were wrong and the task
corrected them.**

This is the task most likely to turn a clean refusal into a wrong answer: a private send the oracle
raises on must not start answering. This paragraph said the corpus gate **cannot see that at all** once
our answer stops being rc 120. It can: our refusal becomes *the oracle's own* 97.2, so the program is
an ordinary differential row. What the corpus still cannot see is a sending position none of its
programs occupies, which is the narrower and true statement.

It also named **table D's `PRIVATE` row** as an instrument. That row is
`gate-tables/directives/method__private__subkeyword.rex`, which declares `::method m private` -- an
*instance* method -- and never sends it, so it witnesses that the directive installs and nothing about
access. **Gate table C's `pubpri` is the table instrument**, and both of its controls redden: allowing
every private send and refusing every private send each take it from `agree` to `diverge-both`, so
neither blanket answer passes. The instruments are `pubpri`, the corpus programs above, and an in-crate
test asserting the refusal shape by scope.
...
program cannot be a 5a differential. **Nor was `self~init:super` a usable substitute when Task 18
ran**, measured there: a message scope override was refused rc 120 on both engines, `a message scope
override on "The Class class" is not implemented (Phase 5)`, because the old plan's Task 5 landed
`Interp::resolve`'s `start_scope` parameter and not the expression that reaches it. **Task 23 built
that expression**, so the refusal is gone and this paragraph's reason is spent; what it decided
stands on its second leg alone. K's superclass is `.Object`, whose class `INIT` is a no-op, so
**dropping the chaining line leaves the oracle's transcript byte-identical** -- measured with and
without, three descriptors read separately -- and that is the program to use:
...
* **`PACKAGE`** -- this paragraph claimed `.context~package~annotation("AUTHOR")` is the only route
  and handed the readback to Task 21. **Both halves are false and Task 20 delivered it**: `.K~package`
  reaches the package object, and `Class~PACKAGE` with the `Package` receiver both predate the task.
  `.context~package` itself is still rc 120 and is still Task 21's.
* **`ROUTINE`** -- this paragraph deferred the readback to 5c because a populated `.ROUTINES` was 5c's.
  **Task 17 populated it**, so `.routines["R"]` and `.routines~r` both resolve here and Task 20
  delivered this readback too.

**Neither split survived measurement, and a third premise fell with them**: the member readbacks were
said to ride Task 9's `~method` alone, when each also needs the Method object to receive a message --
a gap Task 17 parked. Task 20 measured the cost of opening it, found `Method` and `Routine` could
become receivers by the arms `.Package` already had, and built it, so **all six targets read back in
5a**. The paragraphs above are kept because the reasoning they record is what the measurement
overturned; nothing in them should be relied on.
...
### What the first attempt landed, and the five mechanisms it measured as still missing

**Ruling after attempt 1, which resizes row 1 below**: `DO OVER` over a hash collection is built
with a deterministic order of this crate's choosing, and matching the oracle's hash order is a
recorded divergence rather than part of this task. Row 1 carries the reason.

Measured at `5b054d4b5` and landed at `8f7a757e4` and `36c5903c5`. **The install half is done and the
prologue half is not**, and the split is sharp enough to state:

* **The message scope override send** was the ruled-in blocker and is built. With it,
  `CoreClasses.orx` and `StreamClasses.orx` **install completely**: measured, a program made of the
  questions plus both files' directive sections is byte-identical to the oracle on three descriptors,
  on both engines, and `tests/bootstrap_install_oracle.rs` is that differential. `CoreClasses.orx` run
  directly is now the oracle's own rc 159 at its line 47.
* **Checks 3 and 5 already pass** at install level and are asserted against the oracle by that file:
  `.TraceObject~option` is `N`, `.String~hasMethod("DEFINECLASSMETHOD")` and
  `.Supplier~hasMethod("INHERITINSTANCEMETHODS")` are `0`, `.Class~hasMethod("DEFINE")` is `1`.
* **`rexx-lib` is built**, sources and sha256 pins, with the drift control run in both directions.

**The brief's own build list is incomplete in the same way its predecessor was**, and this is the
enumeration a re-dispatch starts from. Each row is measured against the crate at `36c5903c5`:

1. **`DO OVER` a `StringTable`** -- `rexx-exec: one of the interpreter's own objects as a DO header's
   OVER target is not implemented (Phase 5)`, rc 120 on both engines. It is `CoreClasses.orx:63` and
   `StreamClasses.orx:45`: the prologue's first loop is `do name over publicClasses`.
   **Attempt 1 reported this as a blocker on the ground that it needed the oracle's hash order.
   That reading was wrong and Moritz caught it.** Both loops are order-insensitive -- each
   iteration puts one distinct key into `.environment` and one into the package, the resulting
   state is identical under any permutation, and the prologue prints nothing -- so the bootstrap
   needs the iteration and not the order. **The order is a separate and narrower divergence**,
   observable only when a program iterates a table and prints, which no corpus program does because
   the construct answered rc 120 until attempt 2 built it.
   **The order analysis is kept, and what it establishes is narrower than "the order is
   deterministic".** The mechanism is open hashing with **no seed**: one `entries[]` array of
   `bucketSize*2` whose first `bucketSize` slots are bucket heads, overflow off a free chain in the
   upper half, bucket `key->getHashValue() % bucketSize`, `MinimumBucketSize` and
   `DefaultTableSize` both 17 preferring odd (`support/HashCollection.hpp:126` and `:130`),
   iteration bucket-ascending then along each chain (`HashContents.cpp:489`).
   **Whether it reproduces depends on the key type and not on the collection class.** A string's
   `getHashValue` is `31*h + byte` over its own content (`StringClass.hpp:328`) and a number's
   delegates to its string value, so a string-keyed table is fixed: twenty runs of one seven-class
   probe give one sha256, and modelled against the mechanism above, classes `AA BB CC DD EE ZZ MM`
   iterate `DD CC BB AA ZZ EE MM`, which is what the oracle prints, with 17 the only bucket size in
   `2..4000` that reproduces it. Every other key's `getHashValue` is `RexxObject::identityHash`,
   `((uintptr_t)this) ^ UINTPTR_MAX` (`ObjectClass.hpp:340`) -- an address -- so **the oracle does
   not match itself there**: measured, ten oracle runs of one eight-object `IdentityTable` give
   **five** distinct orders, each a rotation of one cyclic sequence as ASLR shifts every bucket
   index by the same delta. Matching an object-keyed order is not expensive, it is impossible, and
   `rust/corpus/README.md`'s determinism rule is where that prohibition is written down.
   **Closing the string-keyed case would also need the crate's own insertion sequence to match the
   oracle's for the table being iterated**, which is true for a package's public classes
   (declaration order) and false for `.environment`, which this crate models as a subset rather
   than building the way `Setup.cpp` does.
2. **`String~UPPER`** -- `rexx-exec: method "UPPER" of class "String" is not implemented (Phase 5)`.
   `CoreClasses.orx:73` needs it twice in one clause.
3. **`Class~defineClassMethod` and `Class~inheritInstanceMethods`** -- absent from the native model on
   purpose (`native_classes.rs`'s `REMOVED_BY_IMAGE_SAVE`), because the *shipped* image does not have
   them and check 5 is that they are gone. The bootstrap needs a state in which `.Class` has them and
   a step that removes them afterwards, which is `Setup.cpp:1809`'s
   `TheClassClass->removeSetupMethods()`.
4. **The `REXX_DEFINED` lock, open during the bootstrap** -- `.string~inherit(.Comparable)` and the
   `~inherit` clauses after it are `98.985 User additions are not allowed to the REXX language
   classes` on both the crate and the oracle today, which is right for a user program and is what the
   prologue has to be allowed to do.
5. **The REXX package accepting additions during the bootstrap** -- `~addClass`/`~addPublicClass` on
   `Package::Rexx` are `Raised::rexx_package_addition` here, the oracle's `checkRexxPackage`
   (`PackageClass.cpp:470`), which tests `isInternalCode()` -- a flag the package does not carry until
   the image is saved.

Rows 3, 4 and 5 are one shape: **a bootstrap-time interpreter state that is not the shipped state**,
closed by one step when the prologue reaches its `exit`. None of the three has a user-visible witness
on its own -- a build that offered any of them to an ordinary program would diverge from the oracle --
so they are landable only together with a bootstrap that exercises them, which is why the first
attempt landed none of them.

**And the run-at-start cost is measured**, since it is the other thing the next attempt has to weigh:
installing `CoreClasses.orx` is **109.9 million `instructions:u`** against **0.59 million** for
`say 1`. Against the guard's axes as they stand that is +3.2% on `alloc4c` (3.47 billion) and +1.2%
on `emptyloop` (9.33 billion), before `StreamClasses.orx` is added to it. The guard asks the task to
say whether a figure at or above 1% is the change or the layout; here it is the change, by design,
and D26's "no image" is the design.

### What attempt 2 landed, and what it cost

**The bootstrap runs.** `Interp::bootstrap_library` runs `CoreClasses.orx` at interpreter start with
its own package object as the argument, `resolve_call` resolves the file's two `CALL`s to the other
two embedded sources while and only while it runs, and
`rexx_classes::remove_setup_methods` closes the bootstrap-time state at the end. All five rows above
are built. **Corpus 246 of 246**, from 242.

Four things the enumeration above did not name and this attempt had to build or fix, each of which
the next reader should not rediscover:

* **`Setup.cpp`'s `InheritInstanceMethods` macro and `Class~inheritInstanceMethods` are different
  functions with the same name**, and this crate had one implementation for both. The macro is
  `RexxBehaviour::inheritInstanceMethods` (`RexxBehaviour.cpp:350`), which copies from the donor and
  leaves it alone; the Rexx method is `RexxClass::inheritInstanceMethods` (`ClassClass.cpp:558`),
  which rewrites the donor's own entries to the recipient's scope. Conflating them was latent until
  something rebuilt the donor's behaviour: measured, `.array~at('x')` then reports `Compiled method
  "AT" with scope "Queue".` where the oracle says `"Array"`, because `Queue`'s block donates from
  `Array`.
* **`::CLASS ... SUBCLASS`/`INHERIT`/`METACLASS` resolved its name against the native table alone.**
  `StreamClasses.orx:506` inherits `Comparable`, which `CoreClasses.orx` declares and its prologue
  puts in `.environment`; the two are separate packages and nothing else connects them.
  `Interp::directive_class` is `PackageClass::findClass`'s order.
* **`removeSetupMethods` cannot rebuild `.Object`'s or `.Class`'s class behaviour with the ordinary
  cascade.** Measured with the root merge left out: `.Object~superClass` is 97.1 where the oracle
  answers `The NIL object`.
* **Every per-run instrument now has a boundary.** `Outcome::collections`, `Outcome::chunks_refused`
  and the compiled engine's own test counters are counted from the end of the bootstrap, because the
  bootstrap is the interpreter starting up and each of those answers a question about the program.

**The cost has two halves and the second is the one nobody had budgeted.**

*The fixed half* is the bootstrap itself: **148.4 million `instructions:u`** per run -- 149.0 million
for `say 1` against 0.59 million for the same program on the pinned pre-bootstrap binary. The
sitting's `fixed` intercept gains that on every axis. It is the change and not the layout, by design,
and it is Task 24's cold-start
number. Profiled: the fixed cost is `MethodDict::add_method` and its hashing during the install
cascade, not the parse (about 4%) and not `remove_setup_methods`'s walk (6.9 million, measured by
deleting it).

*The per-pass half* is the library being **resident**, and it is larger. `per_pass`
`instructions:u`, ir arm, against Task 23's own first sitting on the same pin: `emptyloop` and
`varlookup` **0.00%**, `arith` +3.59%, `dispatchclass` +6.20%, `compound` +10.68%, `alloc4c`
+12.77%, `strings` **+31.35%**. The two flat axes are the two that allocate nothing per pass and the
size of the move tracks how much each of the others allocates, so this is the collector tracing the
library's classes, method objects and `.environment` entries on every collection. **Attributed with
a control build**: the same head source with `bootstrap_library` not called puts `alloc4c` back at
3,606.50 against 3,606.49 and `strings` at 5,406.48 against 5,433.48, so none of it is the scope
override, `String~upper`, the `DO OVER` iteration or the directive-name resolution.

**Task 24 inherits this and its cold-start framing does not cover it.** Whether an interpreter with
its library resident should cost 12% more per allocating pass than one without is a question about
the collector, and it is now measured rather than suspected.

**The release test suite got much slower**, because `ir_dual`'s sweep runs thousands of programs in
process and each pays the fixed cost twice: the slowest test binary is 469 s and the whole command
stopped fitting in the ten-minute window it used to finish inside. No before-and-after total was
taken.
...
**Amended 2026-08-25, at the task's own run: the flip is not in the tree, and the reason is a
measurement.** With the gating arm live -- both under `REXX_PHASE_GATE=5a` and under the
`CLOSED_PHASES = &["5a"]` edit itself, which reddens the identical rows -- the two tables exit 101 on
**five** rows, and each is a 5a mechanism nothing in this plan built:

| row | what it stops at | the task this plan gave it to |
|---|---|---|
| `gate-tables/concepts/usingcl.rex` | `method "MIXINCLASS" of class "Class"`, and `~subclass` behind it | Task 7, per the row's own recorded owning-phase authority |
| `gate-tables/concepts/xscope.rex` | `method "SCOPE" of class "Method"` | Task 9 |
| `gate-tables/concepts/methna.rex` | `a method built from source text`, which is `~define(name, source)` | Task 21 |
| `gate-tables/classes/rexxinfo.rex` | `environment symbol ".REXXINFO"` | the standing `RexxInfo` deferral in `rust/crates/rexx-classes/src/native_classes.rs` |
| `gate-tables/directives/attribute__external__subkeyword.rex` | `::ATTRIBUTE EXTERNAL is not implemented (Phase 7)` | Task 22, which moved `::METHOD ... EXTERNAL 'LIBRARY REXX name'` into 5a and left the `::ATTRIBUTE` spelling of the same `LIBRARY REXX` form behind |

**Committing the flip would put five standing reds under the two corpus gate commands**, which this
plan's global constraints forbid in terms -- "There is no standing red and no exception to read past,
so the two corpus gate commands exit zero" -- and a later task could then no longer tell its own
regression from the five. So the constant stays `&[]`, and the flip is the single line
`pub const CLOSED_PHASES: &[&str] = &["5a"];` in
`rust/crates/rexx-exec/tests/gate_tables/mod.rs`, to be applied by whichever task closes the last of
the five. **Cost if that call is wrong:** the line is not in the tree and someone has to remember it,
against a gate every later task reads past.

**The hierarchy edge rows changed instrument in the same task, and that is not part of the five.**
They asked `~superClasses~hasItem(.Parent)`; `Array hasItem instance` is a **5c** row of
`corpus/docs/class-methods.txt` and `ArrayClass::hasItemRexx` is native (`memory/Setup.cpp:740`), so
a 5a criterion could not be answered until 5c landed. The derived probe now walks the list and
compares `~id`, which is the fidelity `~makeString` and the class rows already have. The criterion
itself was already met: measured before the change, with the membership line removed so this crate
reaches the rest of each probe, all 57 edges agree with the oracle on all three descriptors.
...
**Three more, added 2026-08-25, that belong to neither list and that 5b and 5c both inherit.**

* **A measured, undecided cost.** Loading the library costs every allocating axis **per pass** --
  Task 23's Concern 1, which says it "is a question about the collector's cost model with a large
  resident set" and that the decision was not its to take. The measurement is committed:
  `bench-baselines/phase-5a-arms.tsv`'s last rows are task `23-attempt-2-control-no-bootstrap` at
  `cfffc7899`, and on `strings`, ir arm, `per_pass` `instructions:u`, the pin reads 5,369.53, the
  same code with the bootstrap suppressed reads 5,406.48, and the shipped build reads 7,136.86. A
  `per_pass` figure is already divided by the pass count, so the fixed 148 million cannot move it.
  **What is open is the decision, not the measurement.**
* **`::ROUTINE ... EXTERNAL 'LIBRARY REXX name'` diverges and no gate row can see it.**
  `gate_table_d.rs`'s `owning_phase` files `("::ROUTINE", "EXTERNAL")` to Phase 7, and a row's
  identity is (directive, keyword, position), so one row covers both spellings and its probe picks
  the shared-library form. Measured: `say 'main'` with `::routine r external 'LIBRARY REXX Filespec'`
  is **oracle rc 0** printing `main` against **rc 120** here; calling the routine is oracle rc 168
  with `Filespec` raising its own `88.901` against the same rc 120. That is the `LIBRARY REXX`
  spelling D37 moved into 5a for `::METHOD`. Phase 7 owns the row and nothing is concealed; what
  this records is that the divergence has **no row in either table**, where `::ATTRIBUTE EXTERNAL`'s
  does. Closing it needs `corpus/docs/directive-options.txt` to distinguish the forms.

* **`DO OVER` a `StringTable` iterates in this crate's order and not the oracle's**, and that is
  the one thing 5a shipped that is a *silent wrong answer* rather than a loud refusal. Measured, rc 0
  on every side: a program whose five `::METHOD` directives are `zebra alpha mango delta kiwi` and
  whose body is `do i over .methods / say i / end` prints `MANGO ALPHA KIWI ZEBRA DELTA` on the
  oracle and `ALPHA DELTA KIWI MANGO ZEBRA` on both engines here. The divergence is documented at
  the site (`run.rs`'s `hash_collection_indexes`, which opens by saying the order is this crate's and
  derives what closing it would take) and `corpus/README.md` carries the rule that no corpus row may
  print an identity-keyed iteration order -- so nothing is concealed and no row depends on it. What
  it has never had is a phase against its name: it is in neither list above, and no gate row sees it,
  which is why Task 24's boundary audit did not find it. Closing it means reproducing
  `HashContents`' bucket order for string keys (`31*h + byte`, `MinimumBucketSize` 17,
  bucket-ascending then chain), which is collection-storage work and not directive work.
...
**Met on every clause but the first, 2026-08-25.** The negative control is recorded, the five gate
commands pass, and the report names each item. The five rows in the table above do not `agree`, so
**5a is not closed**; it closes when they do, and the flip lands in that commit.

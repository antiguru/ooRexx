...
`bench-baselines/phase-5a-arms.tsv` holding its header row and nothing else. The standing is
...
* **5b's** -- `~new`, `init` and `self~init:super` chaining as *instance* construction; `UNINIT`
  ...
  5b can run on the day it starts -- two of them silent divergences on this crate today, so 5b does
  not have to look for an instance to see the firing; per-object methods (`SETMETHOD`, `ENHANCED`,

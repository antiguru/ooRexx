...
2026-08-17, or re-measured against the post-rebase tree in the 2026-08-20 round below. No individual
figure is stamped, because the two rounds agree on every figure checked in both.

**The 2026-08-20 rebase took that provenance commit off the branch, and a reader must not take the
paragraph above on trust without it.** `fbeb18db4` is **not an ancestor of the branch head**. The
rebased twin of the commit this plan was written against is `db983f637`, and its `rust/` tree is not
`fbeb18db4`'s: the rebase landed the performance work *underneath* the 5a commits, so a diff between
the two spans that work. The crate figures were therefore re-run on 2026-08-20 against a `rexx-run`
built from the post-rebase tree -- `.array~id`, `(1,)~size`, the `makeString` silent divergence, the
`UNKNOWN` name arm, `::ANNOTATE`'s readback, `.K~method("C")`, `.STEM` as an environment symbol and
`say s. + 1` -- and every one reproduces byte for byte, so the figures below stand. **Two crate
citations did move with it and are corrected here**: the `::ANNOTATE` comment and arm are
`rexx-exec/src/lib.rs:1285`-`1287` and `:1288`-`:1292`, and `body.rs`'s `size_of::<Body>` assert is
`:292`. Both were right at `fbeb18db4`, which is the whole point -- **a `file:line` into `rust/` in
this document is a pre-rebase reading unless it says otherwise**, and `oracle.rs:189`/`:214`,
`owners.rs:251`/`:259` and `tests.rs:1901` were re-checked and still hold. The per-task re-measurement
rule at the end of this section is what depends on neither provenance.

...
| `.context~package~annotation("AUTHOR")`; `.routines["R"]~annotation("AUTHOR")`; `.K~method("A")~annotation("AUTHOR")` for an `::ATTRIBUTE` and `~method("C")` for a `::CONSTANT` | oracle rc 0 `moritz` on every one; crate rc 120 at `.context~package`. **The ownership this row drew from that did not survive Task 20's own measuring, and the reading is what stayed true.** All six targets read back in 5a: Task 17 populated `.ROUTINES`, so the `ROUTINE` route resolves here, and `.K~package` is a second route to the package object that this row did not consider, so the `PACKAGE` readback needs no `RexxContext`. `.context~package` itself is still rc 120 and is still Task 21's -- see Task 20's report |

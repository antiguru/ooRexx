...
program cannot be a 5a differential. **Nor is `self~init:super` a usable substitute**, measured at
Task 18: a message scope override is refused rc 120 on both engines, `a message scope override on
"The Class class" is not implemented (Phase 5)`, because the old plan's Task 5 landed
`Interp::resolve`'s `start_scope` parameter and not the expression that reaches it. K's superclass is
`.Object`, whose class `INIT` is a no-op, so **dropping the chaining line leaves the oracle's
transcript byte-identical** -- measured with and without, three descriptors read separately -- and
that is the program to use:

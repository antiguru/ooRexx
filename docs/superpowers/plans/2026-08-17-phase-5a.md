...
**Entry:** met. The standing is `perf-baseline.md`'s **"The post-optimisation baseline against a 5.3
oracle, measured 2026-08-20"**, and no earlier section of that file: the sections before it were
measured against a 5.0 oracle built `-O0` and a pre-optimisation crate, and that file's own banner
rules every ratio in them void. **The plan restates no number from it** -- it points, so that a
re-measurement there is not a correction here.
**Written against** the crate source at `5de2a8801` and the oracle
`REXX-ooRexx_5.3.0(MT)_64-bit 6.06 30 Jul 2026`; corpus **105 of 106** against that oracle, with the
one red named under Global constraints. Every figure here was re-measured on 2026-08-20 -- see
"What I ran".
...
* **Crate** -- `rust/target/release/rexx-run` built from the crate source at **`5de2a8801`**, which is
  also the crate source at the branch head: no commit since touches `rust/crates`, `rust/Cargo.toml`
  or `Cargo.toml`, checked rather than assumed.
...
**Every figure in this document was re-measured on 2026-08-20 against the pinned pair, both arms, and
one moved.** The corpus is **105 of 106** rather than 106 of 106, and the red is
`corpus/lang/loop_control_rounding.rex` -- 5.3 applies `NUMERIC FUZZ` to a controlled loop's bound
test where 5.0 did not. That is Phase 4 loop control, recorded as a standing red outside Phase 5, and
**no task here owns it or gates on it**. Everything else -- the whole table below and every measured
claim in the task texts -- reproduces byte for byte on both arms, including the `62`/`38`/`24`
constructibility sweep, the `98.942` and `98.985` frame lines, `93.965`, `97.2`, `97.3`, `90.998`,
`99.906`, `99.945`, the `REPLY` exit-status-0-with-traceback transcript, and the `makeString` silent
divergence.
...
`tests.rs:1901` were re-checked and hold.
...
own probes before it starts, and records all three pins when it does**: a figure here is what that
pair answered on 2026-08-20, and the tasks ahead of it in the sequence are expected to move some of
them.
...
The corpus is **105 of 106** against the pinned oracle and must stay there and grow. A task that adds
a corpus program adds it to `rust/corpus/phase-5a.txt` in its own commit.

**The one red is named, so that "the corpus is green" never has to mean "except the one everybody
knows about".** `corpus/lang/loop_control_rounding.rex` fails because 5.3 applies `NUMERIC FUZZ` to a
controlled loop's bound test and this crate implements 5.0's behaviour. It is **Phase 4 loop control,
owned outside Phase 5**: no task here fixes it, Task 24 does not gate on it, and a task that reports
"105 of 106" with that program as the failure has a green corpus. A *second* red is a regression.
...
```
./target/release/rexx-arms --build pinned=bench-baselines/pinned/rexx-run-<pin-commit> \
...
**The pin in that command line must be rebuilt before Task 1, and the pin this plan was written with
is the fifth instance of the defect the whole document is being swept for.**
...
`d6870a358`, checked. So the guard as pinned reads *"no 5a task may be slower than the tree was
before the optimisation work"*, and it has that entire body as headroom to burn: a 5a task could
give back every gain in it and the guard would stay green. **Nothing about the control is badly
...
**What the replacement pin must be**, stated so it can be produced and checked rather than assumed:

* a **release `rexx-run` built from the crate source at the commit Phase 5a starts from** -- today the
  crate source at `5de2a8801`, which is the branch head's;
* stored as `bench-baselines/pinned/rexx-run-<commit>`, **named for that commit**, so the artifact's
  own name carries its pin and a stale one cannot be silently reused;
* with a recorded provenance line beside it -- commit, build date, profile, and the **sha256 of the
  binary**, which is what `perf-baseline.md`'s 2026-08-20 section does for both of its builds and is
  the fix for this whole class of problem;
* and `bench-baselines/phase-5a-arms.tsv` starts empty against the new pin: **a row measured against
  the old pin is not comparable with one measured against the new one**, and a delta across that
  boundary is the same mistake as a ratio across the oracle swap.

**The staleness test, which each task runs before trusting the pin:** the pin's commit is an ancestor
of the task's base, and every commit between them is a Phase 5a task commit. A rebase, or any
unrelated crate work landing beneath the phase, fails that test -- and the pin must then be rebuilt
and the baseline restarted rather than reasoned about.
...
makes it stale and how the task checks that before trusting it.** That covers the pinned binary, the
baseline TSV, `perf-baseline.md`'s standing, the committed row sets, the oracle behind
`oracle_root()`, and every `file:line` into `rust/` or `interpreter/`.
...
**Read-only trees.** `/home/moritz/dev/repos/ooRexx/interpreter/`, `ootest/` and `oodocs/`. Nothing
in this plan writes to any of them.
...
gate commands, corpus unchanged at 105 of 106 with the same program red, and the oracle invocation
count unchanged.
...
* **The UNINIT propagation flags.** The spec's cell reads "5b, flags carried by 5a's code", and the
  ...
  flags**, and 5b owns everything that reads them. Nothing in 5a can observe a flag: `UNINIT` fires at
  object destruction and there are no instances, so **the instrument is an in-crate test that the flag
  propagates through all three constructors, and nothing else** -- no differential, no gate row. Named
  here so the split has both halves owned rather than being handed whole to 5b, which is the D46
  violation the spec exists to prevent.
...
* **`PACKAGE`** -- the only route is `.context~package~annotation("AUTHOR")`, and **the Package
  object is Task 21's**, which follows this task. Crate today: rc 120 at `.context~package`. So this
  task installs the target and **Task 21 owns its readback**, which its "Done when" carries.
...
`PACKAGE` target readback arriving at the task that builds the object it needs**. `~enhanced` is
5b's -- it builds an instance.
...
* the corpus, at or above 105 of 106 with `loop_control_rounding.rex` the only red, and the count of
  programs `phase-5a.txt` gained. **That program is Phase 4's and this gate does not wait on it** --
  if it has been fixed by then the corpus is 106 of 106 and the report says so;
...
* **cold start and D2**, measured here for the first time in this phase:
  ...
  `perf-baseline.md`'s **"Fixed per-process offset (`startup.rex`)"** in its 2026-08-20 section --
...
* **5b's** -- `~new`, `init` and `self~init:super` chaining as *instance* construction; `UNINIT`
  ...
  carried by its constructors; per-object methods (`SETMETHOD`, `ENHANCED`, `unsetMethod`) and the
  object-own scope they create; `FORWARD` and therefore `DELEGATE`; abstract-**class** enforcement;

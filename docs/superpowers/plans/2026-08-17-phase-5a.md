...
**Written against** `fbeb18db4`, tree clean, corpus **106 of 106**.
...
**Every crate-side figure in this document -- in the table below and in every task text -- is measured
at `fbeb18db4`**, on a `rust/target/release/rexx-run` rebuilt from the clean tree at 13:27 on
2026-08-17. There is no second provenance anywhere in this plan, which is why no individual figure is
stamped.

The first pass of these probes ran against a binary dated 12:11, and `d756c64c4` (committed 12:14)
changed `rexx-exec/src/lib.rs`'s staged gap check -- so **that binary could not be assumed to be
...
Oracle figures are from the shipped C++ build at
`/home/moritz/dev/repos/ooRexx/build`, unchanged throughout. **Every task re-measures its own probes
before it starts**: a figure here is what the tree answered before the phase began, and the tasks
ahead of it in the sequence are expected to move some of them.
...
| | item | owner |
|---|---|---|
| **M1** | `::ANNOTATE`'s refusal arm and its comment at `rexx-exec/src/lib.rs:1275`-`1281` | **Task 20** |
...
| **M5** | the environment-symbol order gains the package-local directory, and the four directories gain the entry-method mechanism | **Task 17** |
...
The corpus is **106 of 106** at `fbeb18db4` and must stay green and grow. A task that adds a corpus
program adds it to `rust/corpus/phase-5a.txt` in its own commit.
...
Tasks run **1 through 24 in order**. Three exceptions are not reorderings but statements of what
...
* **Verdict failures are red under `REXX_CORPUS_GATE=1`, and only for rows whose owning phase is
  ...
  of 5a rows not yet `agree`**, which must never increase.
...
**Done when** the ledger is committed and every row carries a stopping point and a revision. No code,
no sitting; the five gate commands still run.
...
* `class-methods.txt` -- one row per (class, method, arm). Every hard case the spec lists must be
  handled and each is a named case in the code: entity-reference prefixes glued to names
...
**The task's own completeness evidence, and what it is not.** Every derived method row is run against
the oracle and **must answer on one arm or the other** -- `.X~hasMethod("M")` or
`.X~new~hasMethod("M")`. A row answering on neither is a defect in the extractor, not a finding about
the crate. **An arm-agreement measurement cannot witness this class of defect at all**, because a
name the oracle does not have has no arm to disagree about; that is why the previous round's
...
**Runnable now, with one dependency:** the sweep sends a bare `~new` to class objects and the spec
records that `.Ticker~new(1000, msg)` constructs and never terminates. Task 1's deadline is what
makes that survivable inside `cargo test`; the opt-in construction programs the criterion allows are
not written here.
...
**Done when** every file is committed with its extractor and its stamp, every derived method row
answers on one arm or the other with the neither-arm set empty, and the `ArgUtil` assertion is in
place. No sitting: no `src/` of a measured crate changes.
...
1. make the crate accept a still-refused keyword silently: the row's `loud` clears and its verdict
   ...
   `subclass.is_some()` admits `MIXINCLASS` at rc 0. The demonstration runnable **here** is that the
   row does not go green under that mutation; the full discriminator is `.M~baseClass`, which is
   `The Object class` for a mixin and `The P class` for a plain class on the oracle, both rc 0 with no
   `~new`, and which the crate can only answer from Task 7 on.
2. **the table types no expected oracle bytes at all** -- every one is re-read from the oracle on the
   ...
   runnable half is to perturb the crate's answer on one row and confirm the row reddens.
...
**Done when** every row has a probe and a verdict, the report prints in both modes, the three
mutations are recorded, and the five gate commands pass. No sitting.
...
**The four falsifying mutations, and none of them can be run in this task.** A mutation control
demonstrates a row going from green to red, and **a row that is already red cannot demonstrate
anything** -- measured, `.array~id` is rc 120 today, so every wiring and scope row is red the moment
it is created. Each control is therefore owned by the task where its row first reads `agree`, and
each of those tasks carries it in its "Done when":

1. drop a class from the registry -- its wiring row cannot answer and reddens. **Task 9.**
2. answer an own-scope query from a flattened all-scopes dictionary -- measured,
   ...
   answers where the oracle raises. **Task 9.**
3. implement a section's mechanism silently wrongly -- the concept row compares three descriptors
   under `Raw`, so `makeString` returning the wrong string reddens at rc 0. **Task 14.**
4. drop the operator-frame line -- the row computes `diverge-stderr`. **Task 6.**
...
`phase-5a.txt` and `RAW_STDERR_COMPARISON` in this commit. Negative control: drop the frame line and
the concept row for the traceback computes `diverge-stderr` rather than `agree` -- the cell the first
taxonomy did not have.

**What it cannot see.** A frame emitted with the wrong scope name is caught only where a probe raises
in a class whose name differs from the receiver's -- name one such program explicitly rather than
assuming the `String` cases cover it.
...
**Done when** the merge-order program answers `parent` on both engines, the diamond is green, a
negative control walking the chain instead of merging reddens it, and `METACLASS` still refuses
loudly with a message naming only what it still refuses. Sitting required.
...
**Done when** the wiring rows for the classes this crate registers read `agree`, and **table C's first
two verdict mutations are recorded as run here**, which is the first point at which either can flip a
green row: dropping a class from the registry reddens its wiring row, and answering `~method` from the
flattened behaviour reddens the scope rows. Sitting required.
...
Directory `~put`/`[]`, both engines. `LoopKind::Over`'s ownership move edits all five `owners.rs`
pinned items in this commit.
...
* **`PRIVATE`: a live over-refusal, fully reachable.** `say .K~m` with `::METHOD m CLASS PRIVATE` is
  ...
  -- is what pins the rule to caller scope rather than to something coincidental. Also: a *sibling*
  class in the same package, which is outside the defining scope and must refuse, and a subclass's
  method sending a superclass's private method, whose answer is measured rather than assumed.
...
**The negative control this task owes table C.** Delete the `makeString` limb and record that the
`reqstr` concept row reddens. **This is the second of D50's two required controls**, and it can only
be run here.
...
program, and the deletion control is recorded. Sitting required.
...
**Goal.** M5 and M6: the eight-step environment-symbol order minus its two package-crossing steps, the
Directory entry-method mechanism, and the `.METHODS` join.

**Build:**

* **The package-local directory as a search step**, above `.LOCAL` and `.ENVIRONMENT`. Measured in the
  spec: with `MYTHING` set in all three, `say .MYTHING` is `from package local`. Steps 3 and 5 cross a
  package boundary and are 5c's, named here so the split is not silent.
...
program in this phase and their rows stay 5c's.

**Done when** the three-way shadowing program, the Directory entry-method program and both `.methods`
forms match byte for byte on both engines, and a control that resolves `.MYTHING` from `.LOCAL` first
reddens the shadowing row. Sitting required.
...
and the floating-form rule, which already agrees at `99.906` rc 157.
...
and a control that creates only the class-side getter reddens the row. Sitting required.
...
**`rexx-exec/src/lib.rs:1275`-`1277`**, whose sentence is true of the program it names and sits where
...
**Verification, runnable now.** Oracle-differential per target and per readback form, both engines,
plus the unknown-target case staying at `99.945` rc 157 byte for byte.

**Done when** all six targets install and read back on both engines, the unknown-target refusal is
unchanged, and the three carriers of the generalisation are fixed. Sitting required.

---

## Task 21: the class-graph mutators, the REXX_DEFINED lock, and the Package object

**Goal.** `~define`, `~defineMethods`, `~delete`, `~uninherit`; the REXX_DEFINED lock that refuses all
five on the interpreter's own classes; and the Package object the bootstrap is handed.
...
`addPublicClass`, `publicClasses` and `~name`. `~enhanced` is 5b's -- it builds an instance.
...
four mutators work on a user class, and a control that skips the copy in `define` reddens the
class-side row it can. Sitting required.
...
matches byte for byte, and invoking an unimplemented entry is loud and names its phase. Sitting
required.
...
2. the post-bootstrap state answering Task 9's wiring questions for every class the prologue installs
   -- which is table C's wiring half, re-run against a bootstrapped state rather than a fresh one;
...
**Done when** the driver exits 0 with empty stdout and stderr on both engines and all five checks
above match the oracle. Sitting required.
...
* **the wiring half of the class-set criterion**: every class named by a `cls*` section across the
  reference, minus `RegularExpression` and minus `RexxInfo`-as-an-environment-entry, plus `ArgUtil`,
  is an `.environment` class answering `~id`, `~class`, `~superClass`, `~superClasses`, `~metaClass`
  and `~isA(.Class)` byte-identically; and each documented edge appears in that class's
  `~superClasses`;
* the corpus, at or above 106 of 106, with the count of programs `phase-5a.txt` gained;
...
* **cold start and D2**, measured here for the first time in this phase:
  `rexx-bench/src/bin/rexx-time.rs --warmup 10 --runs 50` against the C++ median of 5.119 ms recorded
  in `perf-baseline.md`;
...
* **5b's** -- `~new`, `init` and `self~init:super` chaining as *instance* construction; `UNINIT` and
  its propagation flags; per-object methods (`SETMETHOD`, `ENHANCED`, `unsetMethod`) and the
...
* **5c's** -- `::REQUIRES` with `LIBRARY` and `NAMESPACE` and namespace-qualified class references;
  **`>N>`**, whose only route is one of those references; `::OPTIONS` and the `OPTIONS` instruction;
  `::RESOURCE` and `.RESOURCES`; `::ROUTINE`'s option surface and `.ROUTINES`; `Package~local` and

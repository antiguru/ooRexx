...
`rexx-core/src/body.rs:294` is `const _: () = assert!(size_of::<Body>() <= 80);`, so a violation is a
...
four mutators work on a user class, and **the control is the lock rather than the copy**: removing the
REXX_DEFINED check makes `.Array~define("ZORK", …)` succeed at rc 0 where the oracle raises, and the
program reddens.
...
* **the wiring half of the class-set criterion**: every class named by a `cls*` section across the
  ...
  **`RexxInfo` is a requirement, not an exclusion** -- its `.environment` entry must be an *instance*
  whose `~class~id` is `RexxInfo`, since `EndSpecialClassDefinition(RexxInfo)` routes the class object
  to a target that is not reachable as an environment symbol. An earlier draft wrote "minus
  `RexxInfo`-as-an-environment-entry", which reads as dropping the row rather than changing what it
  asks;

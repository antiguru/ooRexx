...
* **Check formatting with `cargo fmt --all --check`, from `rust/`.** Not `cargo fmt --edition 2024 --check`: `cargo fmt` has no `--edition` flag and that spelling exits 2 with `error: unexpected argument '--edition' found`, so a task that runs it and reports formatting clean has verified nothing. It happened in Task 0 and the reviewer caught it. `--edition 2024` belongs to the **standalone** `rustfmt` binary, which does default to 2015 and does reject let-chains -- so `rustfmt --edition 2024 FILE` is right and `cargo fmt --edition …` is not. `cargo fmt` reads the edition from `Cargo.toml` and needs no flag.
* **Read every verification command's exit status unpiped, and check the command ran at all.** A shell pipeline reports the last command's status, so `cargo clippy | tail` in an `&&` chain reports `tail`'s success. A misspelled flag exits non-zero *before* doing any work, which reads as failure in one context and is silently skipped in another. This project has now shipped six checks that could not fail; two of them were verification commands that did not execute.
...
**`rexx-core` may need amending after all, and the cost of avoiding it has doubled.** The redirect can live in `Interp` -- slot resolution returning a `(SlotFrame, usize)` pair, every site changing, and a check added to a path that is 8.1%/32.2% of runtime -- or in `RootSet`, which amends `rexx-core`. This is **Task 5's decision, made with a measurement**. The first revision of this plan asserted `rexx-core` is amended in no task and listed `stem.rs` as "comments only"; both were wrong.

**Re-counted at `344677e6`, after Tasks 0-3 landed: 26 non-comment sites resolve the frame from the top activation, 12 of them in `stem.rs`** -- `eval.rs` 3, `stem.rs` 12, `plan.rs` 4, `lib.rs` 2, `run.rs` 5. The figure recorded when this decision was written was 12 total with 7 in `stem.rs`, taken before the activation stack existed. **Do not cost the `Interp` route from the old number.** Re-count before deciding; it has moved once already and Task 5 is not the last task to touch these files.
...
The witness already exists and is already correctly absent from the 4a subset: **`rust/corpus/lang/interpret_dynamic.rex`**. It exercises a dynamic fragment, a fragment that assigns to a variable, and a fragment inside a `DO` body.

**It does not exercise the `Activation::extra` path, and an earlier revision of this plan said it did.** That claim was false and was copied into the corpus file's own header before a reviewer instrumented `slot_of` and measured zero hits. The program's line 3 is `v = 0`, so `V` is in the enclosing body's plan and the fragment's assignment finds an ordinary slot. Since `extra` exists precisely for "a fragment binds a name the enclosing body never mentions", the property that motivates the design must be witnessed differentially: **append `interpret "zork = 42"` and `say zork`**, measured to match the oracle byte for byte, and check the header claims only what the program does. List it in `phase-4b.txt` with a header in the same shape as `phase-4a.txt`'s, saying which constructs the 4b subset admits and which it still excludes (the corpus rules in Task 10 are the list).
...
* Pointed at `ootest/ooRexx/base/keyword`, the existing extractor yields **54 rows from 2,561 `self~assertSame` calls -- 2.1%**. **Thirty-four of the 39 groups yield zero**, and nine of those 34 contain no `assertSame` at all, so the sharper statement is **25 of the 30 groups that contain any**. Only five groups produce a row: `DO` 3, `NUMERIC` 28, `REPLY` 1, `TRACE` 1, `VarRef` 21.
...
* The cause of that gap is real, not a counting artifact, and it has **two** parts. **403 of the 2,561 calls are not at the start of a line** and `scan_method_for_assertions` only ever tests `trimmed.starts_with("self~assertsame")`. `base/keyword` writes multi-clause lines (`id=0010; ABBREV     =0010; self~assertSame(abbrev, id)`, `ASSIGNMENT`, 226 of them) and trailing assertions (`When i=0 Then self~assertSame(...)`, `IF`, 126 of them). `base/expressions` never does, so the blindness has never shown. The remaining **2** unseen calls are at the start of their line but sit in `GUARD.testGroup`'s `waiter_multiple` (lines 329-330), a `::method` whose name does not begin with `test`, so no scanner ever reaches its body. Calls outside any test method need their own drop reason, or the conservation law cannot cover them.
...
**That shape is reachable in 4b, and this is the measurement that says so.** Redone with `extract`'s own method splitting, comments stripped, exact `assertSame` spelling: **1,132 test methods carry 2,436 exact `assertSame` calls** (2,441 less the 2 outside any test method and 3 inside comments). Of those, **1,034 methods / 1,942 calls** have a body whose only `~` is `self~assert*` -- plain classic Rexx. So a body-shaped extractor needs no message dispatch, and Phase 5 is not a prerequisite. No assertion call uses a line continuation, which removes one hazard. "No message send" remains necessary and not sufficient -- a body free of `~` can still use a BIF we lack or an environment symbol -- so treat these as a ceiling to report against, never as a promised row count.

**Take the narrower population: bodies whose only `~` is `self~assertSame` exactly, which is 896 methods / 1,773 calls, 72.6% of 2,441.** The wider `self~assert*` population buys **+169** `assertSame` calls, but only by replacing **659** other `self~assert*` calls with `nop` -- silently deleting 659 real checks from bodies the table would then report as *passing*. A pass over a body with its other checks removed is a vacuous pass, and 169 rows does not buy that.
...
* `rust/corpus/keyword-exempt.txt`, 796 rows -- **790 of them are `4c` and fire at 4c's own gate**, each body starting to pass and failing the set with `now PASSES but is still on the committed exempt list -- remove it`. The opposite direction is `is failing (...) and is not on the committed exempt list`. Both live in `rust/crates/rexx-exec/tests/keyword_assertions.rs`; **grep the message strings rather than trusting a line number**, since that file is the one a fix round would edit. The other **6** rows are `defect:compound-do-control-variable` and fire whenever that defect is fixed, which is unscheduled.

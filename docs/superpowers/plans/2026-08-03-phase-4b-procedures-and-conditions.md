...
**That shape is reachable in 4b, and this is the measurement that says so.** Redone with `extract`'s own method splitting, comments stripped, exact `assertSame` spelling: **1,132 test methods carry 2,436 exact `assertSame` calls** (2,441 less the 2 outside any test method and 3 inside comments). Of those, **1,034 methods / 1,942 calls** have a body whose only `~` is `self~assert*` -- plain classic Rexx. So a body-shaped extractor needs no message dispatch, and Phase 5 is not a prerequisite. **A line-continuation hazard does exist**, contrary to this plan's first revision: `NUMERIC::test_digits_fuzz_default` (2 calls) and `NUMERIC::test_form_default` (1) both split a `self~assertSame(..., self~runDynamicSource( -` across lines, and Task 11's committed drop table records them as `on a continued line -- 2 methods, 3 calls`. The extractor drops them with a reason rather than mis-parsing them, so the hazard is handled, not open. "No message send" remains necessary and not sufficient -- a body free of `~` can still use a BIF we lack or an environment symbol -- so treat these as a ceiling to report against, never as a promised row count.

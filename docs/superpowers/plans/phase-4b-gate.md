...
Measured, **removing one line at a time**: for **nine of the twelve, that one
deletion leaves `coverage.rs` green**, because the coverage property needs only
*some* program to construct each variant and by the end of 4b most variants have
several witnesses. Only `call_expression`, `use_arg_forms` and `push_queue`
construct something nothing else in the union does.

**That is a claim about each deletion separately, and the collective version is
false** — measured, removing all nine at once *does* fail `coverage.rs`, exit
101, `3 in-scope variant(s) unwitnessed by the phase subsets: Interpret, Signal,
Raise`. The nine are individually unpinned, not collectively invisible, and the
distinction matters because it is the *single silent deletion* the pin exists to
stop.

The worst case was `lang/condition_traps.rex` — **criterion 8's only named
witness, and the declared corpus catcher for one of criterion 6's mutations
(row 6, `SIGL` off by one)** — whose deletion left the corpus reporting
`41 of 41 matching` at exit 0, `coverage.rs` green and `collect_stress` green:
three criteria still reporting MET with one criterion's entire subject removed,
and the headline shrinking 42 → 41 while still reading as a clean sweep. That
whole-suite check was run for `condition_traps.rex` specifically; for the other
eight, only `coverage.rs` was measured. **A "N of N matching" harness cannot
notice a missing program.** The pin closes it, and is verified to fail by name.

*One is the measured number, and an earlier version of this paragraph said
three.* Each of the six corpus-catching mutations diverges on exactly one
program (`41 of 42` in every case), so no program can be the declared catcher
for more than one; row 6 names `lang/condition_traps.rex` and rows 1 and 7 are
structurally out of its reach — it contains no `PROCEDURE`, and its single
`call raiser` fires the trap once where row 7 needs a second raise, which is
why `call_on_trap_rearms.rex` exists. **This was C1's own defect repeating one
round later**, in the paragraph whose entire subject is a falsification clause
...
**Five mutations do not go through the corpus. Four of those are by design; the
fifth, row 2, was declared to be caught there and was measured surviving** —
this gate found that the corpus cannot see trap inheritance, rather than
choosing not to look. The four by design are the queue's storage and order
(criterion 9), a dropped GC root (criterion 4), and an omitted argument's `>A>`
line — none of which a differential run can observe. Classifying those against the corpus would report `PASSED` —
...
union. Every **tagging** `match` still has no wildcard arm — all seven are
generated by `owners.rs`'s `tags!` macro and that file contains no `_ =>` at all
— so a new variant in any of the seven enums is a compile error there rather
than a silently shrinking check.

**Two `match`es in the witness loop do carry `_ => {}`** — the ones inside
`every_in_scope_variant_is_witnessed_by_the_phase_subsets`'s `each_instruction`
and `each_expr` visitors, in `coverage.rs` — and the sentence above deliberately
does not cover them. They are
*dispatch* matches — which sub-enum to descend into from an `InstructionKind` or
an `ExprKind` — so the compile-error property still holds through the exhaustive
`instruction_tag` and `expr_tag` that run first on every node. What they do not
give is sub-enum coverage for a *new* looping or expression-bearing variant: a
future `InstructionKind` that carries a `LoopKind`, or an `ExprKind` that
carries an `Operator`, would be tagged correctly and then silently skipped for
the inner enum until someone adds an arm.
...
**And "the collector" is now observed rather than inferred**, which it was not
in the first two rounds of this document. `mutate-4b.sh` reports the failing
**target binary** for every suite divergence, and row 9's is
**`tests/collect_stress.rs` and nothing else** — not `unittests src/lib.rs`,
not `tests/trace_oracle.rs`. Before that, a `DIVERGED` verdict named only the
failing test functions, and this criterion's central claim rested on reading
those names and concluding which instrument they belonged to. The same output
now separates the two queue rows the same way: row 8's failing target is
`tests/trace_oracle.rs` alone, and row 10's is `unittests src/lib.rs` alone.
...
| `a1484211` (the tip the final whole-branch review measured) | 1,983 | 15,841 | **12.5%** |
...
interpreter lines, and `coverage.rs` grew 55 across the gate's two fix rounds —
48 for the new `phase_4b_subset_matches_the_committed_list` at `0e14fac4`, and 7
more at `a1484211`, a commit whose entire content is corrections to false
claims. The fraction moves 12.2% → 12.5%, which changes
nothing about the argument — a gate's own instrument growing is not the
attribution surface tracking construct count — but labelling a row with a
commit at which its numbers are false would be the same defect this step is
about.

**Every row is labelled with a commit, and none of them says "now", because
"now" is the one label that cannot be written down here truthfully.** The
fourth row was originally "after this gate's own commits", 1,976 — true when
written and false one commit later, at `a1484211`, which added those seven
lines of corrected comment. The final whole-branch review caught it, and the
fix round that corrected it moved the number a third time. **The property is
the durable form of this row**: the harness sits near an eighth of the
interpreter, and it got there by the interpreter growing rather than by the
harness shrinking. Re-measure either total at any commit with

```bash
for f in tests/owners.rs tests/loud.rs tests/coverage.rs; do
  git show <commit>:rust/crates/rexx-exec/$f
done | wc -l          # swap in src/{run,eval,error}.rs for the interpreter
```
...
`rust/` source**: `8d6790c6` touches `keyword-exempt.txt`,
`keyword_assertions.rs`, `keyword.rs` and `extract_keyword.rs` across **406**
insertions — its 480 is the six-file commit total, and the other 74 are
`l1-coverage.md` and this exclusions file — and `b23986d9` touches **three** of
those same four, all but `extract_keyword.rs`. The tree moved substantially
between Task 11 and this gate — just not in the six files this ratio measures,
which is the only thing the empty diff shows. And **exactly one** commit
corrected this step's
...
**The trend runs against the argument consolidation was raised on.** From
`e4caa7bf` to `a1484211` the harness grew **7%** (1,852 → 1,983) while the
interpreter it attributes grew **76%** (9,025 → 15,841). A fixed cost that
shrinks as a fraction while its subject nearly doubles is a *weaker* case for
consolidation than the mid-4b snapshot suggests, and quoting only the 20.5%
figure would put a thumb on the scale.
...
interpreter** — concretely, if the three files exceed ~2,600 lines (a 31% jump
against 4a-to-4b's 7%) or if the fraction *rises*. Measure the rise from
**12.5%**, this step's fourth row, and not from the 12.2% third row: the two
differ only by the gate's own documentation commits, and a few tenths of a point
is what a comment correction costs. What would mean something is a *sustained*
rise across 4c's own construct work, because that would say the attribution
surface has started tracking construct count rather than sitting flat — the only
shape under which "one sub-phase at a time" is genuinely paying per construct.
...
**It is guarded, and the guard is real**:
`every_out_of_scope_variant_fails_loudly`
runs each witness through `run_program` and asserts stderr **ends with**
...
* **One harness hole found by this gate's own review and closed**:
  `phase-4b.txt`'s twelve entries were pinned by nothing, so **any one of nine
  of them could be deleted on its own and leave `coverage.rs` green** — and for
  `lang/condition_traps.rex`, measured across the corpus and `collect_stress`
  too, the whole suite. (Deleting all nine *together* does fail `coverage.rs`;
  the hole is the single silent deletion, not the collective one.)
...
* **The final whole-branch review found the same shape five more times**, in
  files no task had reason to reopen: `l1-coverage.md` still called the
  compound-`DO` gap unowned after this gate assigned it, `tests/support/mod.rs`
  still split the trace prefixes ten/nine after Task 9 added three emitters,
  `coverage.rs` carried a justification its own file falsified 167 lines later,
  the "unavoidably a third copy" claim survived in **two** places while the
  deferred-minors roll-up recorded one, and Step 3b's fourth ratio row was
  false at the commit it shipped in. All five are corrected; the geometry is
  always the same, which is that **the task that falsifies a sentence is never
  the task that owns the file containing it.**

## What 4c inherits, explicitly rather than implicitly

From `.superpowers/sdd/2026-08-03-phase-4b-procedures-and-conditions/phase-4b-final-review.md`.
Recorded here, in a plan document, because a finding left in a review summary
is read once and then lost — `rust/CLAUDE.md`'s "correct the plan, not the
message that carries the work".

**Two findings deliberately not fixed in 4b's final round**, both mechanical,
both deferred because that round was scoped to false statements only:

* **I4 — `read_subset` is three byte-identical hand-maintained copies**
  (`tests/collect_stress.rs`, `tests/corpus.rs`, `tests/coverage.rs`), and only
  `coverage.rs`'s has a test. Criterion 1's "42 of 42" is produced by all three
  agreeing. Task 2b created `tests/support/mod.rs` for precisely this hazard
  and moved one helper into it; this one was left duplicated. **Fix:** move it
  to `support`, add `mod support;` to the two files lacking it. No behaviour
  change. Note that the three doc comments above the copies differ and only the
  bodies are identical, so a mechanical move must not silently drop the parts
  that are not shared.
* **M-c — `rust/corpus/README.md`'s "Current programs" table is not an
  inventory.** 18 of the 50 `corpus/lang/*.rex` files are absent, 12 of them
  4b's own additions. Nothing asserts README completeness, which is why it
  drifted silently and why re-listing without a check would only reset the
  clock. **Fix:** either assert the table against the directory, or drop its
  inventory framing.

**The deferred minors ruled 4c.** The review's triage table carries a ruling
for all 21 rows of `deferred-minors.txt`: **11 are ruled 4c, 8 drop, 2 were
fixed before merge.** Re-derive that split rather than trusting a total. The
review's own summary line says twelve 4c and seven drop, and the cause is
visible in the table: **T1-1** is ruled *drop* and its reason ends "re-derive
in 4c if it recurs", so a search for `4c` moves it across and leaves the drops
one short. Matching the bolded ruling instead of the free text:

```bash
R=.superpowers/sdd/2026-08-03-phase-4b-procedures-and-conditions/phase-4b-final-review.md
awk -F'|' '/^\| \*\*T/ && $5 ~ /\*\*4c\*\*/ {print $2}' $R     # 11 rows
awk -F'|' '/^\| \*\*T/ && $5 ~ /\*\*drop/     {print $2}' $R   #  8 rows
```

That is a small error and it is recorded because of its shape, not its size:
a count derived by searching prose for a token counted a row that argues
*against* being in the set. The rulings themselves are unaffected.

| # | finding | ruling, verbatim from the triage table |
|---|---|---|
| T0-M1 | Step 5's block sits at end of file, not the module doc | cosmetic placement |
| T0-M2 | `expand_for_witnesses` not exhaustiveness-checked against `Call`/`Signal` | a renamed arm degrades to a silent no-op expansion; real but remote |
| T0-M3 | `the_two_harnesses_include_this_exact_file` defeated by the regression it names | it catches deletion of the `#[path]` line, not a hand-copied table added *beside* it. Fixing needs a different property |
| T0-M4 | `instruction_arm` returns `String` where `&'static str` suffices | mechanical |
| T1-3 | a mutation-kill note naming a mutation that cannot be typed | a mutation that cannot be applied is an unfalsifiable kill claim; same family as criterion 6's `INFRA_FAILURE` class |
| T1-5 | ungrammatical `Outcome::collections` rewrite | prose only |
| T1-7 | Step 4's test duplicates an existing assertion without saying so | see also I3, which is the same test's *other* comment |
| T1-8 | a tripwire cost claim relying on dead-code elimination | an unmeasured performance claim; either measure it or delete it, per `rust/CLAUDE.md` |
| T7-c | `RAISE PROPAGATE`'s report rendering rests on four transcripts, not a family | an evidence-depth concern, not a known defect; fold into 4c's condition work |
| T7-d | `run_activated`'s `_program` parameter is dead | test helper, one line |
| T7-e | `trap_for` is `pub(crate)` with no cross-module caller | narrow to private |

The remaining ten rows need no 4c work: two were fixed in the final round
(T0-M5, and T1-4's shape as I1/I2/I3/M-a), and eight were dropped — four of
those because the tree had already closed them, which is what a roll-up nobody
re-checks looks like from the inside.

**`deferred-minors.txt` itself is not a safe input.** It was generated by
`grep -n 'minor (deferred)' progress.md`, which misses Task 7's two blocks
twice over: they head with a capital `Minor`, and they are prose blocks
carrying five items inside running text, so no line-grep recovers the items
even case-insensitively. The five were appended by hand and the file now
carries a header saying so. Anything 4c generates the same way needs the same
warning on it.

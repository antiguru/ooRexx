...
- **Phase 4** is where the execution model is fixed. Read the existing performance profile before designing the dispatch loop. The 81 builtins from Task 0.6 are the checklist; tick them off individually.
  ...
  **Phase 3 hands over three obligations, recorded here because they have nowhere else to live.**
  ...
  * **`TRACE`'s value lines are Phase 4's, and gated on a decision.** `*-*` source lines are Phase 3's; the fifteen value markers all carry an evaluated value and need an executor. Emitting an event per evaluation step forbids constant folding and fusion, so decide that before designing the dispatch loop rather than after. `TRACE.testGroup` holds 239 expected trace-output lines, which is the cost of the deferral.

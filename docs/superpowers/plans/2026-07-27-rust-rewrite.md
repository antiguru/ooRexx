...
- **The subsystem's documentation is an entry condition for planning it, and its absence is a blocker rather than an inconvenience. Recorded 2026-08-17 by Moritz, after Phase 5a.** Do not write or review a phase plan until the authority for that subsystem is checked out and read. The ooRexx documentation is in the same SourceForge SVN repository as the test suite -- `^/docs/trunk` beside `^/test/trunk`, DocBook XML -- and Phase 5's spec and plan were both written without it.
  - **What that cost, measured rather than asserted.** Phase 5a's plan was reviewed by a four-reviewer panel, revised, executed through seven completed tasks, then re-reviewed twice more. Only the pass that read the documentation found that **two of `provide.xml`'s thirteen sections -- `UNKNOWN` and Required String Values -- are absent from the 5a plan and from the Phase 5 spec entirely**, and that both are **silent wrong answers at rc 0** rather than loud refusals: `say .K` with a `::METHOD makeString CLASS` answers `K says hello` on the oracle and `The K class` here, with matching exit status and empty stderr on both sides. No gate in this project can see that.
...
**No global interpretation lock in the end state. Recorded 2026-08-16 by the user as a design constraint, and it binds every phase, not only Phase 6.** (a) remains the delivery order: Phase 6 **may** ship a coarse lock as an implementation stage. What the constraint forbids is anything *depending* on one. No type may assume single-threaded interpretation, no invariant may rest on "only one activity runs at a time", and an API that is sound only while a global lock is held is a **defect rather than a shortcut** — it is to be reported as such in review, in Phases 1 to 5 as much as in Phase 6. This sharpens the sentence above it ("structure the interpreter loop so the lock *can* later be split") from an intention into a rule a reviewer can apply to a diff, which is the difference between a constraint that holds and one that is discovered to have been violated once splitting is attempted.

**Why "nothing may depend on it" is the load-bearing half.** A lock's presence in the source is not evidence that it runs: on the C++ side, static-initialisation order made `ResourceSection` a no-op for the whole process lifetime, measured on this repo. That is a *different* lock from the kernel lock and says nothing directly about the kernel lock's soundness — what it demonstrates is the failure mode this constraint is written against, where code is correct only under a serialisation that turns out not to exist. The known defects listed above are the same shape: a cross-thread yield path reaching into another activity's frames is unsound whether or not a kernel lock happens to be held.

**What this does not settle.** Whether Phase 6 *ships* fine-grained locking is still (a) versus (b), and (b)'s conformance risk against ooTest groups that encode observable ordering is unchanged by this constraint. Reversing the delivery order is a separate decision and would rewrite Phase 6's exit gate.
...
*Which file, measured 2026-09-15.* Two builds of that extension exist, both from identical sources
(`diff -rq extensions/rxregexp /home/moritz/dev/repos/ooRexx/extensions/rxregexp` prints nothing),
and the sentence above is true of one instrument. The corpus differential hands both interpreters
the oracle checkout's `/home/moritz/dev/repos/ooRexx/build/lib/librxregexp.so` (Build ID
`1aa47dd9...`, 126,408 bytes): that is the `{oraclelib}` a `.env` sidecar expands through
`tests/support/sidecar.rs`, so there the oracle and this crate load the same bytes. The in-crate
tests -- `rexx-api/tests/{load,invoke,context}.rs` and the unit tests in
`rexx-exec/src/dispatch/library.rs` -- open this worktree's own `build/lib/librxregexp.so` (Build ID
`afcaedc0...`, 32,392 bytes), a binary the oracle never runs. The `NEEDED` set and the zero `rexx`
imports above hold on both (`readelf -d`, `nm -D --undefined-only`, re-run 2026-09-15), so no
measurement here depends on which one was read.
...
which is what section 6's Phase 8 bullet (the one beginning *"**Phase 8** rebuilds `testbinaries/`
unchanged"*) requires of this phase: *"open a decision block for it before writing the
...
**The D2 coupling, which is the reason this decision is not merely aesthetic.** D2's default is to bootstrap by parsing `CoreClasses.orx` + `StreamClasses.orx` — 5,203 lines of Rexx — at *every* interpreter start. Parser throughput therefore lands inside cold start, as one component of it alongside bootstrap execution, heap setup and class construction. A parser that is pleasant but 5× slower does not just miss a Phase 3 benchmark, it is what forces D2 into building an image cache that would otherwise never be needed. This paragraph is the origin of the "sets cold-start time directly" claim that the corrections list below records as wrong, and it kept asserting it for another two passes. Measure parse throughput on `CoreClasses.orx` specifically, not on a synthetic input.
...
| 8 | Native API | 5, 7 | `testbinaries/` compile unchanged against frozen headers; native-API ooTest groups pass. **The L2 slice closed 2026-09-14**, `docs/superpowers/plans/phase-8-gate.md`: a shared library loads, an extension's methods run, and `docs/superpowers/plans/phase-8-l2.md` records the chain walked again. **L2 is still not reached, and its blocker is this phase's by Moritz's ruling of 2026-09-14.** The framework now gets past `.ENDOFLINE`, past `rxregexp.cls` and past `::METHOD INIT EXTERNAL "LIBRARY rxregexp RegExp_Init"`, and stops at `ooTest.frm:49`, `.local~hasEntry(...)`. Nine `Directory` methods refuse on `.local` and `.environment` and every refusal names **Phase 5**, which is closed; D-L2 below recorded that no open phase owned them, and Moritz then ruled that this phase does, as the surface half's first task. The surface half of this phase (the function pointers the L2 slice does not fill, `testbinaries/`, and the three API groups whose binaries import nothing from the interpreter -- `METHOD`, `CONVERSION` and `FUNCTION`; the other five are rows 9 and 10's, measured 2026-09-15 and recorded there) has its own plan | L2, blocked |
| 9 | Core conformance | 6, 7, 8 | **L3-core on the host**: the full suite green against existing baselines *excluding* the groups enumerated below; every benchmark at parity; `rexx`, `rexxc`, `rxqueue`, `rxsubcom` ship. The other four platforms are Phase 11's, per D-P1 below. **Four native-API ooTest groups are this phase's, re-homed from Phase 8 on 2026-09-15 by measurement**: `RexxStart`, `ProcessRexxStart`, `INVOCATION` and `ProcessInvocation` each load `INVOCATIONTester.cls` (`.context~package~loadPackage`, line 52 of each group), which binds `orxinvocation`; `liborxinvocation.so` NEEDs `liborxexits.so`, which NEEDs `librexx.so.4` and `librexxapi.so.4` and imports `RexxCreateInterpreter` and `RexxStart` (`readelf -d` and `nm -D --undefined-only` on the oracle checkout's `build/lib`), so running those groups is running the embedding API this row ships | L3-core |
| 10 | RXAPI & extensions | 9 | `rxregexp`, `rxmath`, `rxsock`, `hostemu`, `orxncurses` recompile and pass; RXAPI decision (D7) executed; **L3-full** with no exclusions. **`CLASSIC`, the eighth native-API ooTest group, is this phase's, re-homed from Phase 8 on 2026-09-15 by measurement**: `CLASSICPackage.cls` binds `orxclassic` and `CLASSIC.testGroup:822,861,875` register `orxclassic1` through `rxfuncadd`, and `liborxclassic.so` and `liborxclassic1.so` import the function, subcom, queue and macro-space registries (`nm -D --undefined-only` on the oracle checkout's `build/lib`), which are this row's RXAPI | L3-full |
| 11 | Other platforms | 10 | A second host exists and its own oracle is built there; the platform seam is named and every host assumption sits behind it; the corpus differential, the gate tables and the benchmark axes run **on each platform available**, each with its own recorded baseline; `PARSE SOURCE`'s first word is the host's rather than a constant. Phases 7 and 9's platform clauses close here | L3-full, per platform |

Phases 6, 7, and 8 are independent of each other and may run in parallel once Phase 5 closes.

**D-L2, 2026-09-14: L2's blocker is a closed phase's leftovers, and no open phase owns them.**
*(As recorded when opened; the ruling at its end gives the blocker to Phase 8.)*
Measured at Phase 8's L2 slice close and recorded in `docs/superpowers/plans/phase-8-l2.md`. The
framework stops at `ooTest.frm:49`, `.local~hasEntry(...)`. It is not one method: `hasIndex`,
`entry`, `hasEntry`, `setEntry`, `items`, `index`, `remove`, `supplier` and `allIndexes` all refuse
on `.local` and `.environment`, and only `at` and `put` answer. Every one of those refusals names
**Phase 5**, which closed on 2026-09-09, and `closed_phases.rs`'s own documentation already records
Phase 5's leftover `Directory` refusals as a debt no open phase holds.

*This is a decision, not a task.* Until a phase takes those methods the `Rung` column cannot be
re-homed, because every candidate phase would be carrying a rung it has no work to deliver. That is
the same defect Phase 7's close identified when it moved L2 to Phase 8, and moving it again without
an owner would repeat it rather than fix it.

*What the oracle does on the same command*: runs the group, 33 tests and 11741 assertions, rc 0.
Ours is rc 120 with one refusal line, so the gap is a refusal and not a wrong answer.

**Ruled by Moritz, 2026-09-14: Phase 8 takes those methods.** *"Yes, implement the missing
functions. It's an oversight we didn't do it yet."* They are Task 1 of
`docs/superpowers/plans/2026-09-14-phase-8-surface.md`, ahead of the ooTest groups that need the
framework to load. He recalls a blocker for `.environment` and not what it was, so that task finds
it by running before it designs. Measured when the ruling was made: all nine answer on a plain
`.Directory~new` byte-identically on all three descriptors, and `.environment` and `.local` are
`Body::Native` directories that `dispatch/hash.rs`'s `owns` excludes from the store family.

**D-P1, 2026-09-13 (Moritz): every phase through 10 targets the host, and the other platforms are Phase 11.**
The gates here are differential, so a platform this crate cannot run an oracle on is a platform none of its criteria can be evaluated against. That was recorded inside Phase 7 as "platforms other than Linux -- CANNOT ASSESS", which left Phases 7 and 9 carrying an "all 5 platforms" clause no run could satisfy; those two rows now say the host, and this row says where the rest goes.

*What is deferred is portability, not correctness.* Nothing about the host-only decision licenses a behavioural difference: on a platform the crate does not run, there is no output to differ, so this is not an accepted divergence and must not be recorded as one.

*What unblocks it* is a second host -- a VM or another machine -- which Moritz will provide. Until one exists Phase 11 has no instrument, and a phase whose exit criteria cannot fail is worse than an unwritten one.

*What it will cost, measured 2026-09-13 rather than estimated.* The host assumptions are small and already localised: `PLATFORM = b"LINUX"` in `parse_template.rs`, which `PARSE SOURCE` answers with on every host; eight `std::os::unix` uses across `command.rs`, `lib.rs`, `builtin/rexxutil.rs`, `dispatch/files.rs`, `dispatch/stream.rs` and `bin/rexx-run.rs`; and nineteen `rustix` call sites in two files. `rustix` is the portable half of that set -- its `fs` covers the whole unix family, the same set `interpreter/platform/unix/` covers -- so the seam this phase builds is not a reaction to it.

**Phase 5's class clause was "32 classes exist and respond" until 2026-08-25, and a build holding 32 of the environment's 62 classes satisfied it.** That number is exactly `/bin/grep -acE "^::[Cc][Ll][Aa][Ss][Ss]" CoreClasses.orx`, which answers 32, while the shipped oracle's `.environment` holds 62 class objects -- measured the same day by iterating `.environment` and counting the entries answering `~isA(.Class)`. So a build that never ran `StreamClasses.orx` at all could satisfy it, and "respond" named no question. The replacement is the criterion gate table C's class, edge and `ArgUtil` rows already run (`rust/crates/rexx-exec/tests/gate_table_c.rs`, over `rust/corpus/docs/class-set.txt` and `hierarchy-edges.txt`), so the clause is a set of rows a command answers rather than a count.
...
**Why a letter series and not a numbered phase after 10.** These are an audit axis, not build steps: the BIF work is possible the moment 4c closes, and numbering them after Phase 10 would say otherwise. (Phase 11 is the platform phase, added 2026-09-13; this sentence predates it and named that number only as the one the S series declined.)
...
**Phase 4 is split into six sub-phases**, each with its own spec, plan and gate, because the row above is larger than Phases 0 to 3 combined: **4a** values, variables, expressions and control flow; **4b** procedures and conditions; **4c** `PARSE` and the builtins; **4d-1** measures the interpreter against the Global Constraints `:39` parity gate and attributes the gap (closed 2026-08-09); **4e** builds the IR (closed 2026-08-11 -- the register IR is the default engine, both engines agree across the suite under STRICT, and the arm ratios are recorded on instructions and cycles with the residual itemised, `phase-4e-gate.md`); **4f** runs the optimisation loop to the bar.
**The debt route is closed on classic-Rexx axes.** An earlier wording let 4d "either close the gap or record the shortfall as a debt in the shape D1 and Phase 2 already used"; that is withdrawn for `arith`, `compound`, `strings`, `varlookup`, `alloc4c` and `rexxcps`, and survives only for `dispatch`, `alloc.rex` and `startup`, where it scopes work to Phase 5 rather than conceding a bar. See `docs/superpowers/specs/2026-07-30-phase-4a-executor-design.md`, which records D15 to D19 and the exclusions and rexxcps gate items, and `docs/superpowers/specs/2026-08-08-phase-4d-performance-design.md` for 4d. The Phase 4 row closes when **4f** closes.

**Re-sequenced 2026-08-09, and the reason is structural rather than a scheduling preference.**
4d-1's own conclusion is that the seven causes it attributed, landed perfectly on every axis, reach no axis's parity bar -- the best derived bar is 1.62x, and two axes have no prediction at all.
So the remaining gap needs a structural change, and **anything that restructures the interpreter has to land before the gate closes, or the performance work becomes a patch on top of something already certified.**
...
What the IR does give OO dispatch is the `resolve`/`invoke` pair both engines share and the clause region a send op sits inside; `ClassGraph`'s per-behaviour version is what a send cache would guard on, and no interpreter path reads it -- its only reader is the test that pins its own bumping (`rust/crates/rexx-classes/tests/behaviour_wiring.rs:807`-`:825`).
...
rust/
  ...
  crates/
    ...
    rexx-api/                 # Phase 8: C ABI export surface
      src/ffi.rs              #   the inbound boundary; one of the two modules carrying
                              #   #[allow(unsafe_code)] under D-U1 (2026-09-14), under a
                              #   crate root of deny (not forbid)
      src/load.rs             #   the outbound boundary; the other granted module
...
- **Phase 4** is where the execution model is fixed. Read the existing performance profile before designing the dispatch loop. The 81 builtins from Task 0.6 are the checklist; tick them off individually.
  ...
  **Phase 3 hands over four obligations, recorded here because they have nowhere else to live.**
  * **`resolveCalls` is deliberately not ported.** `translateBlock`'s tail resolves deferred `CALL`/`SIGNAL`/function targets against the label table (`LanguageParser.cpp:1690`). It raises nothing and its effect is purely run-time, and `CodeBody::labels` gives the same information per body, so Phase 3 left it out on purpose. **Whoever owns call resolution must not assume it is done.**
  * **`TRACE`'s value lines are Phase 4's, and gated on a decision.** `*-*` source lines are Phase 3's. Every other marker carries an evaluated value and needs an executor, which is eighteen of the nineteen prefixes in `RexxActivation.hpp:92-110`. Say "everything except `*-*`" rather than listing them: three separate enumerations of that set have been wrong, because `>>>` and `>.>` defeat the obvious character class and two of the eighteen are `<I<` and `+++`. Emitting an event per evaluation step forbids constant folding and fusion, so decide that before designing the dispatch loop rather than after. `TRACE.testGroup` holds **342** expected trace-output lines across 34 `::resource` blocks, 128 of them `*-*` clause lines and 214 carrying a value or a marker, which is the cost of the deferral. The figure recorded here until 2026-07-30 was 239, which matches no counting method; anchor the block scan to `^[[:space:]]*::resource`, because an unanchored match trips on the comment at `TRACE.testGroup:161` that mentions the directive and swallows 51 lines of method code. Two independent recounts produced 393 by exactly that mistake before the anchored one produced 342.
  * **A method-name intern table.** `ExprKind::Message::name` is `Box<[u8]>` rather than a `SymbolId`, because a message name written as a literal (`a~'length'`) never reaches the scanner as a symbol token. Phase 4 will want its own intern table for these.
  * **Nothing in Phase 3 observes AST *shape*, only node counts.** The acceptance tests for the two shipped `.orx` files and Task 3.10's benchmark assertion both compare counts: main-body instructions, directive count, and the total nested inside directive bodies. Because instruction lists are flat and source-ordered, and `If`/`Do`/`Select` hold target *indices* into them, a dropped clause anywhere does change a count, so early stopping and empty directive bodies are caught. Three classes are not: control-flow targets wired to the wrong index with counts unchanged, a body-boundary bug that moves a clause between adjacent directives while the sum holds, and any corruption inside an `Expr`, since no count looks into one. Phase 4 will meet all three as wrong behaviour at run time rather than as a failing parser test, and that is the trade Phase 3 made rather than an oversight. Closing them needs shape observation, a per-directive body-length vector compared verbatim and a checksum over instruction tags and target fields, which is worth building the first time a wiring bug costs a debugging session.
...
- **Phase 5** is the project's inflection point. When `CoreClasses.orx` runs, 32 classes appear at once and the L2 rung becomes reachable. Budget for the fact that it will expose parser and executor gaps in bulk rather than one at a time.
  ...
  **Performance work is suspended for the duration of Phase 5, under conditions decided on 2026-08-14.** Phase 4f left `rexxcps` at about 3.5x the oracle, down from 4.6x, with entry 59 of `phase-4f-record.md` the largest single movement. The decision was to stop there and build the object model rather than keep optimising, because a fast interpreter that cannot run object-oriented Rexx is fast on a subset. What that decision depends on:

  * **Pin the standing as a recorded baseline before Phase 5 changes anything.** Staged at one fixed binary path, interleaved rounds per arm, minimum of each, spread beside the difference -- the same discipline the 4f entries use. This cannot be reconstructed once the tree moves, and two non-interleaved suite runs have already invented a 5% regression that was really a 5% improvement.
...
- **Phase 7**'s stream model is a subsystem, not file I/O. `StreamNative.cpp` is 3,765 lines and its positioning and line/character interaction rules are all observable.
  ...
  **Phase 7 inherits the whole of ANSI's condition-delivery machinery, and nothing before it can test any of it.** The conditions X3.274 delays to a clause boundary are `HALT`, `FAILURE`, `ERROR` and `NOTREADY` (6.3.2.39's `callable_condition` is exhaustive). `ERROR` and `FAILURE` need command issuance and `NOTREADY` needs streams, so all of them are behind this phase: measured 2026-08-15, a `CALL ON ERROR` program exits `rexx-exec: a command is not implemented (Phase 7)` at rc 120 on both engines. Everything this crate delivers today runs on the `RAISE`/`USER` path, which is an ooRexx extension the standard does not contain at all -- there is no `RAISE` instruction and no `USER` condition in it, so those programs are Msg25.1 plus a syntax error under ANSI and say nothing about conformance. The free pre-publication draft is at `https://www.rexxla.org/rexxlang/standards/j18pub.pdf` with its errata at `stan_err.html`; the errata binds any conformance claim.

  What Phase 7 has to decide, each already read out of the draft rather than left to be discovered:

  * **8.2.4 drains a boundary, and so do ooRexx and this crate.** The standard's clause-termination step is `do t=1 to 4` over `HALT FAILURE ERROR NOTREADY`, delivering everything pending at the current level, in that fixed name order -- not in arrival order, and not one per boundary. This bullet used to say that this crate delivers at most one and that ooRexx did too, and to set as Phase 7's work the measuring of ooRexx on a clause leaving two differently-named conditions pending. That measurement was taken on 2026-08-15 and the ANSI spec's opening carries the program and the transcripts: the oracle runs two handlers at one boundary, and so does this crate, on both engines. **The drain itself is therefore no longer a disagreement.** What Phase 7 still owes is the *order*: the programs measured use `RAISE`/`USER`, so whether ooRexx sweeps the four conditions ANSI defines in 8.2.4's fixed name order rather than in arrival order is unmeasured. Running it needs two of the four pending at one boundary, so at least one of `ERROR`, `FAILURE` or `NOTREADY` has to be raisable, which is what puts the test in this phase; `HALT` alone would not.
  * **7.5.3 attributes `SIGL` to the clause in which the condition occurred**, not the clause at whose boundary the handler was entered. The two coincide whenever delivery is at the originating clause's own boundary, and come apart the moment a delivery slips to a later clause. Measured on the oracle with a `CALL ON ERROR` handler that itself issues a failing command: `SIGL` reads the originating clause throughout, so ooRexx agrees with the standard here and this crate must reproduce it once commands exist.
  * **8.4.1 discards a same-name condition raised while its own handler runs**, with one extra `HALT` the sole exception -- `#Enabling` stays `DELAYED` until the handler returns. Measured: the oracle's nested failing command inside an `ERROR` handler runs the handler no second time. An engine that queues it instead diverges from both.
  * **The errata to 8.3.8 and 8.3.22 add a clause boundary at the callee's level on `RETURN`/`EXIT`**, before the level pops, so a condition delayed by a callee's last clause is handled *in the callee*. The errata says this exists for compatibility with existing implementations, which makes it the likely ooRexx behaviour too, and it is the kind of thing an implementation gets wrong silently.

  `docs/superpowers/specs/2026-08-15-ansi-condition-delivery.md` carries the verbatim quotes, the section numbers, the errata reading and what was inference rather than text. It also records where ooRexx departs from the standard, so that matching ooRexx there is a recorded decision rather than an unnoticed bug.

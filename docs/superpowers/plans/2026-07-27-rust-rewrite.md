...
| 4 | Classic executor, split 4a / 4b / 4c | 2, 3 | Non-OO Rexx runs: assignment, `DO` (all variants), `IF`, `SELECT`, `CALL`, `PARSE`, `SAY`, `SIGNAL`, conditions, and **66 of the 81 builtin functions**, the excluded set being exactly `phase-4-exclusions.txt`; `samples/rexxcps.rex` runs clean and within the ratio bar | L0 named subsets + the `base/expressions` assertion table |

...
| 8 | Native API | 5, 7 | `testbinaries/` compile unchanged against frozen headers; native-API ooTest groups pass. **The L2 slice closed 2026-09-14**, `docs/superpowers/plans/phase-8-gate.md`: a shared library loads, an extension's methods run, and `docs/superpowers/plans/phase-8-l2.md` records the chain walked again. **L2 is still not reached, and the blocker is no longer this phase's.** The framework now gets past `.ENDOFLINE`, past `rxregexp.cls` and past `::METHOD INIT EXTERNAL "LIBRARY rxregexp RegExp_Init"`, and stops at `ooTest.frm:49`, `.local~hasEntry(...)`. Nine `Directory` methods refuse on `.local` and `.environment` and every refusal names **Phase 5**, which is closed, so the rung has no owner -- see D-L2 below. The surface half of this phase (the function pointers the L2 slice does not fill, `testbinaries/`, and the six API groups that are not embedding) has its own plan | L2, blocked |
| 9 | Core conformance | 6, 7, 8 | **L3-core on the host**: the full suite green against existing baselines *excluding* the groups enumerated below; every benchmark at parity; `rexx`, `rexxc`, `rxqueue`, `rxsubcom` ship. The other four platforms are Phase 11's, per D-P1 below | L3-core |

...
- **Phase 7**'s stream model is a subsystem, not file I/O. `StreamNative.cpp` is 3,765 lines and its positioning and line/character interaction rules are all observable.
  ...
  * **8.2.4 drains a boundary; this crate delivers at most one.** The standard's clause-termination step is `do t=1 to 4` over `HALT FAILURE ERROR NOTREADY`, delivering everything pending at the current level, in that fixed name order -- not in arrival order, and not one per boundary. `Interp::leave_clause` takes one and does not re-check, which matches ooRexx on the extension path. **Where the two disagree the bar is ooRexx**, so the work here is to measure ooRexx on a clause that leaves two differently-named conditions pending and record which rule it follows, rather than to implement the standard directly.
  ...
  * **The errata to 8.3.8 and 8.3.22 add a clause boundary at the callee's level on `RETURN`/`EXIT`**, before the level pops, so a condition delayed by a callee's last clause is handled *in the callee*. The errata says this exists for compatibility with existing implementations, which makes it the likely ooRexx behaviour too, and it is the kind of thing an implementation gets wrong silently.
  ...
  `docs/superpowers/specs/2026-08-15-ansi-condition-delivery.md` carries the verbatim quotes, the section numbers, the errata reading and what was inference rather than text. It also records the two places ooRexx departs from the standard, so that matching ooRexx there is a recorded decision rather than an unnoticed bug.

...
| 4 | Classic executor, split 4a / 4b / 4c / 4d | 2, 3 | Non-OO Rexx runs: assignment, `DO` (all variants), `IF`, `SELECT`, `CALL`, `PARSE`, `SAY`, `SIGNAL`, conditions, and **66 of the 81 builtin functions**, the excluded set being exactly `phase-4-exclusions.txt`; `samples/rexxcps.rex` runs clean and within the ratio bar (the Global Constraints `:39` parity shipping gate; **not** `:40`'s 1.5× figure, which scopes to Phase 1) | L0 named subsets + the `base/expressions` assertion table |
...
**Phase 4 is split into four sub-phases**, each with its own spec, plan and gate, because the row above is larger than Phases 0 to 3 combined: **4a** values, variables, expressions and control flow; **4b** procedures and conditions; **4c** `PARSE` and the builtins; **4d** measures the interpreter against the Global Constraints `:39` parity shipping gate, attributes any gap, and either closes it or records the shortfall as a debt in the shape D1 and Phase 2 already used. See `docs/superpowers/specs/2026-07-30-phase-4a-executor-design.md`, which records D15 to D19 and the exclusions and rexxcps gate items, and `docs/superpowers/specs/2026-08-08-phase-4d-performance-design.md` for 4d. The Phase 4 row closes when 4d closes.

...
| 8 | Native API | 5, 7 | `testbinaries/` compile unchanged against frozen headers; native-API ooTest groups pass. **L2 arrives here**, not in Phase 7: loading a shared library is the last thing between `ooTest.frm` and its first test group, measured at Phase 7's close | L2 |

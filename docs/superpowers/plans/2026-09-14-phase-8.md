...
records it: the slice `rxregexp` needs comes first, and the remaining surface -- the 211 function
pointers the slice does not fill, `testbinaries/`, and the eight ooTest API groups -- gets its own
plan document written after L2 is measured. That is not a deferral of the gate; it is a refusal to
specify the generalisation before the first real extension has run against the design. Task 11
writes it.
...
* **The C++ tree is read-only**, and so are `samples/`, `build/`, `ootest/` and `oodocs/`. Phase 8
  reads `build/lib/librxregexp.so` and must not rebuild it -- loading a compiled extension is the
  instrument (D5 amendment, 2026-09-14), and a rebuild throws it away. Two builds exist from
  identical sources: the in-crate tests open this worktree's `build/lib/librxregexp.so`, the corpus
  differential the oracle checkout's (`phase-8-gate.md` section 4).
...
- [ ] **Step 4: The method table.** Walk `RexxMethodEntry[]` to its `REXX_LAST_METHOD` terminator,
      ...
      **90.998** and a `::ROUTINE` one is **90.999**, both raised while the directive is installed
      and before the program's first clause runs. `98.978` is reachable from no surface at all:
      measured, `loadExternalMethod` answers `.nil`, `Package~loadLibrary` answers `1` or `0`, and
      `.Object~package~loadLibrary('nosuchlib_zz')` is **98.984** (with no argument it is 88.901
      first).
- [ ] **Step 5: Tests against this worktree's prebuilt binary.** `build/lib/librxregexp.so`, by absolute
...
- [ ] **Step 3: `OPTIONAL_` tolerates an omitted argument** and the others do not. **The numbers
      here are measured against the oracle, not read out of `RexxErrorCodes.h`**, because that file
      has already produced three wrong numbers on this phase: an absent required argument is
      **88.901** and an unconvertible one is **88.909**. `93.968`
      (`Error_Incorrect_method_signature`) and `40.918` are `reportSignatureError`, raised for a
      malformed signature and not for an argument, and neither is witnessed by any run this phase
      can make without compiling an extension of its own.
...
- [ ] **Step 5: The end-to-end test.** `RegExp_Init` from a prebuilt, never rebuilt `librxregexp.so`, called
...
**Interfaces:**
- Produces: `SetObjectVariable` and `DropObjectVariable` behind the method-context table, and
  `WholeNumberToObject`, `StringData`, `StringLength`, `NewPointer` and `RaiseException0` behind
  the thread table -- the pointer names; the scoping survey's section 2 maps the extension's
  spellings `WholeNumber` and `RaiseException0` onto them. (As first written this line put all
  seven behind the method-context table under the extension's spellings; corrected 2026-09-15.)
...
- [ ] **Step 5: `RaiseException`, `RaiseException0`, `RaiseException1`** raise the numbered
      ...
      sub-number, message text and descriptor all as the oracle produces them. [As built,
      `ffi.rs` fills `RaiseException0`, the one `rxregexp` calls; the family's other three slots
      stay at the refusing stub.]
...
- [ ] **Step 5: Witnesses** in `corpus/lang/` for each directive form, and for the load failures
      ...
      **90.999** for a method and a routine the library does not export, and **98.984** for
      `.Object~package~loadLibrary`. Each with its `sourceline_oracle` companion and a `phase-8.txt`
      row.
- [ ] **Step 6: Commit.**
...
- [ ] **Step 2: Write the surface plan** now that a real extension has run: the 211 function
      pointers the slice did not fill, grouped by what they touch, `testbinaries/`, and the six
      ooTest API groups that are not embedding. The two that are embedding are re-homed to
      Phase 9 with the spec's section 9 as the reason. [Measured 2026-09-15 (the scoping survey's
      section 3): three groups are extension-only, four reach the embedding API and one the
      Phase 10 registries; the re-homing was recorded by the docs pass, not by this task.]

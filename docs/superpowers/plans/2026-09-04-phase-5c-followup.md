...
so a stub that reads its arguments still cannot pass it. Add the truncation case. Name it in
`corpus/unfiled.txt` until Task 4 files it.
...
plan. File Task 2's and Task 3's witnesses into the **existing** `corpus/phase-5c.txt`, update
`EXPECTED_SUBSET_5C` in `coverage.rs` which pins that file's exact line list, and empty
`corpus/unfiled.txt` of them.

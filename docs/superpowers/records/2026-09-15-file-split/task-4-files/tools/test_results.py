...
def parse(path):
    ...
    for line in open(path, errors="replace"):
        ...
        if m and block:
            name = m.group(1)
            if block.startswith("doc-tests ") and "--keep-doc-lines" not in sys.argv:
                # A doc-test's name ends in its source line, which any move
                # above it shifts; the item path before it is the name.
                name = re.sub(r" \(line \d+\)", " (line _)", name)
            tests.append(f"{block}\t{name}\t{m.group(2)}")
            continue

...
# located in the files this task touches (PAT).
PAT='--> crates/rexx-exec/src/(lib|tests|refusal|directives|install|variables|run|run/interpret)\.rs'
cd $1
...
for f in $3-doc.txt $3-doc-private.txt $3-doc-cfgtest.txt; do
  echo "$f: $(/bin/grep -a -c '^warning' $f) warning lines, $(/bin/grep -a -E -c -- "$PAT" $f) located in lib.rs, its children and run.rs/run/interpret.rs, $(tail -1 $f)"

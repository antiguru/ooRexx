...
S=/tmp/claude-1000/-home-moritz-dev-repos-ooRexx-rust-rewrite/99c66dfa-1d22-4940-ab62-784c7ef57f5f/scratchpad/file-split-4
{
  ...
  echo "rust tree $(cd $W && git add -A -- rust ':!rust/corpus-l1' ':!rust/bench-baselines/pinned' ':!rust/target' >/dev/null && git write-tree --prefix=rust/)"
  echo "load-before $(cat /proc/loadavg)"
} > $O.meta
(cd $W/rust && CARGO_TARGET_DIR=$T memcap 8G cargo test -j 4 --release -p rexx-exec --no-fail-fast > $O.log 2>&1)
echo "exit $?" >> $O.meta

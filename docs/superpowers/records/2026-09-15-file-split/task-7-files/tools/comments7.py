...
rustlex) and `rust/corpus` (every line, since corpus files are prose):
  ...
  (c) every comment line inside the moved text itself (the destinations)
      that uses a positional or file-relative word ("above", "below",
      "this file", "this module", "here", ...), wherever it names.
...
dests = dests.split(",")
post_src = os.path.join(post_rust, "crates/rexx-exec/src")
...
pos_re = re.compile(r"\b(above|below|earlier|later|this file|this module|here|next to|beside|immediately|following|preceding|further (up|down))\b", re.I)
...
for root, _, files in os.walk(os.path.join(post_rust, "crates")):
    ...
    for f in sorted(files):
        ...
        in_dest = any(rel == "crates/rexx-exec/src/" + d for d in dests)
        local = in_dest or rel == "crates/rexx-exec/src/" + parent
        for n, l in comment_lines(p):
            ...
            if in_dest and pos_re.search(l):
...
for title, hits in (("(a) comments naming a moved unit", hits_a), ("(b) comments naming a touched file", hits_b), ("(c) positional words inside the destinations", hits_c)):
    print(f"\n{title}: {len(hits)}")
    for h in hits:
        print("  " + h)

...
for N in $(seq $1 $2); do
  ...
  [ -z "$(comm -23 <({ git diff --name-only HEAD -- rust; git ls-files --others --exclude-standard -- rust; } | sort -u) <(sort /tmp/claude-1000/-home-moritz-dev-repos-ooRexx-rust-rewrite/99c66dfa-1d22-4940-ab62-784c7ef57f5f/scratchpad/file-split-7/snap$N/files))" ] || { echo "a changed file outside the snapshot"; git status --short; bad=1; }
  [ $bad = 0 ] || exit 1

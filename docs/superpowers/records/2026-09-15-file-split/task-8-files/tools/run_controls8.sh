#!/bin/bash
# usage: run_controls8.sh OUTDIR SUFFIX
...
# Task 7's ten (controls_task7.py), and this task's own (controls_task8.py).
...
python3 $S/tools/controls_task8.py > $O/controls-task8-$X.txt 2>&1; echo "task8 exit $? $(tail -1 $O/controls-task8-$X.txt)"

...
D=$1/crates/${SPLIT_CRATE:-rexx-exec}/src
...
(cd $1 && CARGO_TARGET_DIR=$2 RUSTDOCFLAGS="--cfg test --document-private-items" cargo doc -j 8 --no-deps -p ${SPLIT_CRATE:-rexx-exec} > $3.log 2>&1)

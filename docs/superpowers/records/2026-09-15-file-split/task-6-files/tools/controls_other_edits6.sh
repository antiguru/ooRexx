...
run() { mapfile -t RULES < $S/c$1/rules; python3 $S/tools/other_edits.py $W/rust $S/oe6.out "${RULES[@]}" > /dev/null; r=$?; echo "c$1 $2: exit $r (expected $3); $(/bin/grep -a "FAIL \\|BAD " $S/oe6.out | /bin/grep -v FAILURES | tail -1)"; }

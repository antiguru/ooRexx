...
for rel in files:
    for u in splitlib.load_units(f"{post_dir}/{rel}"):
        # A moved test module's items are keyed with its name before the move.
        k = (os.path.basename(rel)[: -len(".rs")] + "::" if rel.endswith("tests.rs") and rel != parent else "") + u["key"]
        if k in pre_keys:
            where[name(k)] = rel
from poswords import POS_WORDS
WORDS = re.compile(r"\b(" + POS_WORDS + r")\b", re.I)
for rel in files:

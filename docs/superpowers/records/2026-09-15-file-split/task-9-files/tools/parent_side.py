...
for line in r.stderr.splitlines():
    if "error" not in line:
        continue
    path = line.split(":")[0]
    in_dispatch = f"src/{PARENT}" in path
    if path.endswith(f"{PARENT}/{child}.rs"):
    ...
    if m:
        parent.add(m.group(1)); found = True
    m = re.search(r"cannot find \w+(?: \w+)* `(\w+)` in module `(crate::)?" + PARENT + "`", line)
    if m:
        crate_path.add(m.group(1)); found = True
    for m in re.finditer(r"no `(\w+)` in `(?:crate::)?" + PARENT + "`", line):
        (parent if in_dispatch else crate_path).add(m.group(1)); found = True
    m = re.search(r"method `(\w+)` is private", line)
    if m:
        methods.add((m.group(1), PARENT if in_dispatch else "crate")); found = True

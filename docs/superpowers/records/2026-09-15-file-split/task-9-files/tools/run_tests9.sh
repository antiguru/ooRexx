...
# run_tests8.sh for Task 9: instrument 4 over WORKTREE as it stands, the
# brief's `cargo test -j 4 --release -p rexx-parse -p rexx-exec --no-fail-fast`
...
(cd $W/rust && env -u CARGO_TARGET_DIR memcap 8G cargo test -j 4 --release -p rexx-parse -p rexx-exec --no-fail-fast > $O.log 2>&1)
echo "exit $?" >> $O.meta

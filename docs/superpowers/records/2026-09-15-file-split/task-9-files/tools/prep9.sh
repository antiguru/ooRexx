#!/bin/bash
# usage: prep8.sh N
# prep8.sh for rexx-parse: pre<N>/ is the main checkout's HEAD
...
git -C $M archive HEAD rust/crates/rexx-parse/src | tar -x -C $S/pre$N.x
mv $S/pre$N.x/rust/crates/rexx-parse/src $S/pre$N; rm -r $S/pre$N.x

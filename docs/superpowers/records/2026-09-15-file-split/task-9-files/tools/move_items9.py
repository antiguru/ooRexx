...
usage: move_items9.py PARENT_RS CHILD_RS REMOVED_JSON HEADER_FILE [--impl=HEAD] KEY...
...
declaration line, the only edit made to moved text.
...
if rest and rest[0].startswith("--impl="):
    ...
    rest = rest[1:]
widen = {k[1:] for k in rest if k.startswith("+")}
    ...
    if k in widen:
        ...
        for i in range(by_key[k]["first"] - 1 - a, len(tl)):
            if re.match(r"^(\s*)(fn|const|static|struct|enum|type) ", tl[i]):
                tl[i] = re.sub(r"^(\s*)", r"\1pub(super) ", tl[i], count=1)
                break
        else:

#!/bin/bash
# usage: rerun7.sh N...
# Re-runs instruments 1-3 with the final tooling on each Task 7 commit pair:
# PRE is the parent commit's src, POST the commit's, both from `git archive`;
# the arguments are the commit's own (task-7-files/c<N>/args: parent,
...
D=$M/docs/superpowers/records/2026-09-15-file-split/task-7-files
O=$S/art/final/rerun; mkdir -p $O
tree() { [ -d $S/trees/src-$1 ] || { mkdir -p $S/trees/src-$1; git -C $M archive $1 rust/crates/rexx-exec/src | tar -x -C $S/trees/src-$1; }; echo $S/trees/src-$1/rust/crates/rexx-exec/src; }
for N in "$@"; do
  mapfile -t A < $D/c$N/args
  P=${A[0]}; DEST=${A[1]}; T=${A[2]}; OPTS=("${A[@]:3}")
  C=$(git -C $M log --format=%h -F --grep=task-7-files/c$N/ 964a6a8db..HEAD); PC=$(git -C $M rev-parse --short=9 $C^)
  X=; [ "$T" != none ] && X=--tests
  SPLIT_PARENT=${P%.rs} SPLIT_PARENT_RS=$P SPLIT_DESTS=$DEST SPLIT_TESTS_RS=$T python3 $S/tools/instruments.py $(tree $PC) $(tree $C) $D/c$N/removed.json $O/c$N $X "${OPTS[@]}" > /dev/null

"""Instruments 1-3 for one commit of the dispatch.rs split.
...
CHILDREN = sorted(
    ...
    for f in os.listdir(os.path.join(post_root, "dispatch"))
    if f.endswith(".rs")
)
pre_children = set(
    os.path.join("dispatch", f)
    for f in os.listdir(os.path.join(pre_root, "dispatch"))
    if f.endswith(".rs")
)
...
VIS = ("pub(crate) ", "pub(super) ", "pub(in crate::dispatch) ", "pub ")
...
# ---------------------------------------------------------------- units
pre_units = {u["key"]: u for u in splitlib.load_units(os.path.join(pre_root, "dispatch.rs"))}
pre_units_file = {k: "dispatch.rs" for k in pre_units}
post_units = {}
post_units_file = {}
for rel in ["dispatch.rs"] + NEW_FILES:
    for u in splitlib.load_units(os.path.join(post_root, rel)):
        key = u["key"]
        if tests_mode and rel == os.path.join("dispatch", "tests.rs"):
            key = "tests::" + key
        assert key not in post_units, ("duplicate key across files", key, rel)
        post_units[key] = u
        post_units_file[key] = rel

pre_dispatch = read_lines(os.path.join(pre_root, "dispatch.rs"))
post_lines_cache = {}
...
actual = read_lines(os.path.join(post_root, "dispatch.rs"))
sm = difflib.SequenceMatcher(a=expected, b=actual, autojunk=False)
equal_blocks = 0
equal_lines = 0
out1.append("(a) unmoved lines: PRE dispatch.rs minus the removed lines, against POST dispatch.rs")
...
moved = [k for k in pre_units if post_units_file.get(k, "dispatch.rs") != "dispatch.rs"]
moved_ok = 0
reformatted = []
for key in moved:
    pu = pre_units[key]
...
out3.append("whole-file literal multiset, PRE dispatch.rs against POST dispatch.rs + new files:")

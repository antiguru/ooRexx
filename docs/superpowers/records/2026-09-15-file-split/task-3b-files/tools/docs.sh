...
for f in $3-doc.txt $3-doc-private.txt; do
  echo "$f: $(/bin/grep -a -c '^warning' $f) warning lines, $(/bin/grep -a -E -c -- '--> crates/rexx-exec/src/run' $f) located in run.rs and run/, $(tail -1 $f)"
done
# The same with cfg(test) on, which is the only way rustdoc resolves the
# intra-doc links of run/tests.rs and run/tests/ (plain `cargo doc` never
# compiles them).
...
f=$3-doc-cfgtest.txt; echo "$f: $(/bin/grep -a -c '^warning' $f) warning lines, $(/bin/grep -a -E -c -- '--> crates/rexx-exec/src/run' $f) located in run.rs and run/, $(tail -1 $f)"

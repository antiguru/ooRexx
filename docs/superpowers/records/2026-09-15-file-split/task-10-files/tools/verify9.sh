#!/bin/bash
# usage: verify9.sh N PARENT_RS DESTS [instrument options]
# verify8.sh for rexx-parse: the per-commit checks (checks9.sh), a snapshot of
...
# failure; commit_task9.sh commits afterwards.
...
$S/tools/checks9.sh "$@" > $S/art/checks-c$N.out 2>&1

...
# (checks7.sh), a snapshot of every changed file under rust/, instrument 4 on
# a test worktree holding exactly HEAD plus that snapshot, and the results.
# Stops at the first failure; commit_task7.sh commits afterwards.
...
N=$1
df -h /tmp | tail -1
$S/tools/checks7.sh "$@" > $S/art/checks-c$N.out 2>&1

#!/bin/bash
# usage: perf9.sh BASE_BIN HEAD_BIN OUTDIR JOBS  (perf8.sh on this brief's four programs)
...
mkdir -p $OUT/cwd
PROGS="$R/bench-rexxcps/rexxcps.rex $R/bench-programs/nop.rex $R/bench-programs/startup.rex $R/bench-programs/parse.rex"
jobs=()

...
//! `static` or `const` whose value is an array of tuples (the native method
//! tables);
...
fn item_name(item: &Item) -> String {
    match item {
        ...
        // A macro item that declares statics (`thread_local!`) is keyed by
        // their names as well, so several of them in one file stay apart.
        Item::Macro(i) => {
            let toks: Vec<String> = i.mac.tokens.clone().into_iter().map(|t| t.to_string()).collect();
            let statics: Vec<&str> = toks
                .windows(2)
                .filter(|w| w[0] == "static")
                .map(|w| w[1].as_str())
                .collect();
            if statics.is_empty() {
                format!("macro {}", i.mac.path.to_token_stream())
            } else {
                format!("macro {}/{}", i.mac.path.to_token_stream(), statics.join(","))
            }
        }
...
fn collect(items: &[Item], prefix: &str, out: &mut Vec<Unit>) {
    for item in items {
        ...
        match item {
            ...
            // A `const` table is rowed as a `static` one is: `hash.rs` and
            // `collection.rs` declare their native tables `const`.
            Item::Static(_) | Item::Const(_) => {
                let (ident, expr) = match item {
                    Item::Static(st) => (&st.ident, &st.expr),
                    Item::Const(c) => (&c.ident, &c.expr),
                    _ => unreachable!(),
                };
                if let Expr::Reference(r) = &**expr {
                    // A table of tuples is one unit per row; any other array
                    // (a list of names) is one unit, the whole static.
                    // A `const` is rowed only when every row is a tuple whose
                    // first two elements are string literals, so every key
                    // names its row; any other `const` is one unit.
                    let str_lit = |e: &Expr| matches!(e, Expr::Lit(l) if matches!(l.lit, Lit::Str(_)));
                    let const_rowable = |arr: &syn::ExprArray| {
                        arr.elems.iter().all(|e| {
                            matches!(e, Expr::Tuple(t) if t.elems.len() >= 2
                                && str_lit(&t.elems[0]) && str_lit(&t.elems[1]))
                        })
                    };
                    if let Expr::Array(arr) = &*r.expr
                        && arr.elems.iter().any(|e| matches!(e, Expr::Tuple(_)))
                        && (matches!(item, Item::Static(_)) || const_rowable(arr))
                    {

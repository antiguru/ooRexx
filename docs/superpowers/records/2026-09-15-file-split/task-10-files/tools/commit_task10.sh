...
N=$1
cd $M
D=docs/superpowers/records/2026-09-15-file-split/task-10-files
mkdir -p $D/c$N $D/instrument4 $D/tools/item-tool/src
cp $S/art/c$N-* $D/c$N/; cp $S/c$N/removed.json $D/c$N/removed.json
for f in move.txt comment-pass.md rules spec.json args where extra keys header pre.txt; do [ -f $S/c$N/$f ] && cp $S/c$N/$f $D/c$N/; done
...
mkdir -p $D/base; cp $S/art/base/* $D/base/

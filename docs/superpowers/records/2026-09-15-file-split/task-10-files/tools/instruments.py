...
# directly before a closing `]` or `}` dropped on both sides, and anything
# else it changed fails. Rust gives such a comma no meaning there. A comma
# before `)` is not dropped: `(x)` and `(x,)` are a parenthesised
# expression and a 1-tuple (Task 7's review, M2).
...
# ---------------------------------------------------------------- units
def use_key(u):
    """A `use` unit's key with any comma directly before a `}` dropped:
    rustfmt drops the trailing comma of an import list it joins onto one
    line (a de-indented `use a::{\n    b,\n};` becomes `use a::{b};`), and
    the key is the declaration's tokens. Only the key: the tokens, compared
    by instrument 2, keep the comma, so such a unit still differs there
    unless it is a declared reflow (`--expect-reflow`), which admits a
    comma before `]` or `}` and nothing else. A `use` key holds no literal,
    so the substitution cannot reach one."""
    if re.match(r"^(\w+::)*use ", u["key"]):
        u["key"] = re.sub(r",\}", "}", u["key"])
    return u


_pre_list = [use_key(u) for u in splitlib.load_units(os.path.join(pre_root, PARENT_RS))]
...
for rel in [PARENT_RS] + NEW_FILES:
    ...
    had = {use_key(u)["key"] for u in splitlib.load_units(os.path.join(pre_root, rel))} if rel in EXISTING_DESTS else set()
    for u in map(use_key, splitlib.load_units(os.path.join(post_root, rel))):
        key = u["key"]
        if key in had:
            continue
...
for key in moved:
    ...
    if ok:
        ...
        reformatted.append(key)
        out1.append(f"  WHITESPACE-ONLY (declared reflow) {key} -> {rel}: {msg} (rustfmt reflow; a comma before a closing `]` or `}}` ignored; tokens are instrument 2's, literal values instrument 3's)")
...
for k in common:
    if pre_units[k]["toks"] == post_units[k]["toks"]:
        ...
        tok_ok += 1
        out2.append(f"  CLOSING-COMMA-ONLY (declared reflow) {k}: identical once every comma directly before a closing `]` or `}}` is dropped on both sides")

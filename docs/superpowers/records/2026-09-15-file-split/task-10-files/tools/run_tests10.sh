#!/bin/bash
# usage: run_tests10.sh WORKTREE OUTPREFIX
# run_tests10.sh for Task 10: instrument 4 over WORKTREE as it stands, the
# brief's `cargo test -j 4 --release -p rexx-extract -p rexx-exec --no-fail-fast`
...
(cd $W/rust && env -u CARGO_TARGET_DIR memcap 8G cargo test -j 4 --release -p rexx-extract -p rexx-exec --no-fail-fast > $O.log 2>&1)
echo "exit $?" >> $O.meta

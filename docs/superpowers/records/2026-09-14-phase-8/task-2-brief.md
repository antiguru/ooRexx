...
- [ ] **Step 4: The method table.** Walk `RexxMethodEntry[]` to its `REXX_LAST_METHOD` terminator,
      keyed by name. **The error numbers are measured, not guessed, and the ledger carries the
      transcripts**: a `::METHOD ... EXTERNAL` naming an entry the table does not carry is
      **90.998** and a `::ROUTINE` one is **90.999**, both raised while the directive is installed
      and before the program's first clause runs. `98.978` (`Error_Execution_library_method`) is a
      different path -- `PackageManager.cpp:947` and `:968`, reached by `loadExternalMethod` and
      `Package~loadLibrary`, which is Task 8's.

...
### 4d-2 -- superseded 2026-08-09 by Phase 4f, and by Phase 4e before it

**This section is withdrawn. Both wordings are kept, per this spec's own amendment rule.**

It read that 4d-2 would be planned from the attribution once 4d-1 closed, as one task per cause, each naming the axes it must move and by how much, with a stopping rule of "every axis meets its bar, or a task's measured result contradicts the attribution".

Two things falsified it, both measured by 4d-1 itself.

**A one-cause-per-task list cannot survive its own first task.**
The attribution measures `compound` as C2 41.5%, C1 14.2%, C4 12.7%, and separately measures the small-integer `//` fix at **-51.6%** on that axis.
Land it and every remaining share there is wrong, because the denominator moved.
A linear plan would spend its second task optimising against a model its first task invalidated.
Optimisation is search, not construction: the candidate queue is regenerated by re-profiling, not consumed in order.
See `docs/superpowers/plans/2026-08-09-phase-4f-optimisation-loop.md`.

**And the causes do not reach the bar.**
4d-1's own conclusion is that landing all seven, perfectly, on every axis leaves four axes between 1.62x and 2.80x and two with no prediction at all.
So a phase planned solely from them runs out of causes before it runs out of gap, and the remaining gap needs a structural change rather than another local fix.
**Phase 4e, the IR, therefore precedes the optimisation loop**, because performance work that lands after a gate closes becomes a patch on top of something already certified.

**The bar is also raised, and narrowed.**
Within noise of the oracle or better, on every classic-Rexx axis, with **no recorded debt available on any of them** -- `arith`, `compound`, `strings`, `varlookup`, `alloc4c`, `rexxcps`.
The debt mechanism `phase-4d-gate.md` defines survives only for `dispatch`, `alloc.rex` and `startup`, where it states scope rather than concedes ground.
"Within noise" is contingent on the band being small: `phase-4d-gate.md` currently declares 7.2%, derived as the largest of three observations, and Phase 4f's Unit 0 re-derives it from a characterised distribution before any of this is decidable.
...
* The seven SIGABRT programs and the 512 MiB address-space reservation at `rexx-exec/src/lib.rs:309`. That reservation is a per-process startup cost and may become relevant to `startup`; if the attribution names it, it re-enters. It is *not* "D19's 512 MiB reservation" -- D19 decides a sized thread and deliberately records no number.

**4d-1 corrects a subject reversal about that reservation.** `perf-baseline.md:262` is its origin, and the documents that follow it inherited it: they say the **oracle** reserves the 512 MiB and this crate reserves none. The reverse is true. The set is not enumerated here, because an enumeration in prose rots and this one already did -- a draft of this paragraph named a report whose mentions turn out to be attribution-neutral and were never wrong. 4d-1's Task 1 owns finding them and records what it found. `INTERPRETER_STACK_BYTES` is this crate's own constant and appears nowhere in the C++ tree, and the behaviour is direct: measured 2026-08-08, `say 1` under `ulimit -v` exits 0 on the oracle at a 100 MB cap and fails here (rc 101) at both 100 MB and 400 MB, succeeding only at 600 MB.
So under the project's standard `ulimit -v 1048576` it is **this crate** that has roughly 500 MB of room while the oracle has roughly 1000, which is what `rust/CLAUDE.md` has said correctly all along.
The reversal inverts which interpreter is disadvantaged by the shared cap, and every memory finding taken under that cap should be re-read against it -- the seven SIGABRT programs' analysis already assumed the correct direction, having worked from the code rather than from the prose.
* `dispatch`, to Phase 5.

...
**4d-1 corrects a subject reversal about that reservation, which is wrong in four documents.** `perf-baseline.md:262` and, following it, `rexxcps-measurement.md`, `task-5-report.md` and `phase-4-exclusions.txt` all say the **oracle** reserves the 512 MiB and this crate reserves none. The reverse is true. `INTERPRETER_STACK_BYTES` is this crate's own constant and appears nowhere in the C++ tree, and the behaviour is direct: measured 2026-08-08, `say 1` under `ulimit -v` exits 0 on the oracle at a 100 MB cap and fails here (rc 101) at both 100 MB and 400 MB, succeeding only at 600 MB.

...
| the roadmap's first-listed reason for the IR — that it "founds OO dispatch for Phase 5 by making a call site a patchable slot" | **closed 2026-08-22 by Phase 5a's Task 10, which amended the sentence rather than recording a reason it survives.** D28 sends no send through that slot: `Op::Message` carries no `site` field where `Op::Call`'s `site` is the classic-call cache. The line, still at `2026-07-27-rust-rewrite.md:492`, now reads "it makes a classic call site a patchable slot" and the two lines under it carry the correction; the old spec cites `:482` |

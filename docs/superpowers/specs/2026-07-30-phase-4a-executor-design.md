...
* **Numeric-or-string comparison**: `= \= <> >< > < >= <= \> \<`, where `<>` and `><` are `\=` aliases.
  **Call `rexx-num`'s free `compare`. It is the whole algorithm, string fallback included, and it is already correct.**

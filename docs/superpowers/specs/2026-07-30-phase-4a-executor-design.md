...
**A third recount will get 374 and conclude this figure is wrong. It is not.** Two independent recounts already went astray on the anchoring above, producing 239 and 393; the next trap is case. The file also contains six **uppercase** `::RESOURCE` blocks that a case-sensitive scan does not see: three embedded `.rex` programs and three `*_expected` blocks carrying 32 further expected lines, whose prefixes are `>I>` and `<I<`, the invocation pair, which is 4b territory and beyond. A case-insensitive scan of the directive alone gives **40 blocks and 437 lines**; narrowing that to the 37 blocks and 374 lines of *expected trace output* additionally requires dropping the three uppercase blocks that hold embedded `.rex` programs rather than expectations. An earlier version of this sentence attributed that second step to the scan itself, which is the same shape of error the sentence exists to prevent, found by an audit that ran the scan rather than reading the claim. All four counts are correct answers to four different questions. 342 is the figure for the lowercase-directive blocks that 4a's decision was sized against, and the 32 extra lines change nothing here because none of them is 4a-runnable either. Whoever recounts should state which scan they used rather than which number they got.
...
2. **The `base/expressions` assertion table passes**, every row evaluated, the count reported, and **each row compared byte for byte against the expected value, never numerically** — a numeric comparison would hide the entire created-digits and created-form story across thousands of rows, which is the defect class D15 exists to prevent.
   ...
   **CONCATENATION (388) needs more than a triple and must not be added naively.** Every one of its assertions references variables `a` through `g` assigned at the top of the test method, which a row of (expression, expected, digits) cannot carry — and **some** of the failure is silent rather than loud: with `a`..`g` unset, every one renders as a distinct single-character name, so a row whose expected value happens to match that all-distinct pattern coincides and passes while testing nothing. Measured, that is the strict `==` and `\==` rows, 106 of the 388. The other 282 use non-strict `=` and depend on real blank-padding equalities between different variables, so they fail visibly instead — line 71 expects `0 0 1 1 0 1 0` and unset operands do not produce it. Either way every row needs the real prelude to mean anything; the point is that the group cannot be added naively, not that all of it fails quietly. A row therefore carries the method's **assignment prelude**, and any assertion whose prelude cannot be represented is listed as blocked rather than included. **Corrected: the split is 106 silent and 282 loud, not 56 and 332, and the mechanism stated here was also wrong.** All 56 strict rows are silent *plus 50 non-strict ones*, so "the silent set is the strict rows" does not hold. Measured against the group itself; the plan carried the same error and commit 4f282fdc fixed only the plan, leaving this governing copy stale.
...
4. **The named L0 subset passes again under collect-on-every-allocation**, and **the mode is proved to do something before its pass is believed**. This criterion was one sentence with no anti-vacuity device while every other criterion here has one, which made it the only one of the seven that could not fail: `Heap::alloc_with` never collects today, its `collect` has no caller outside `rexx-core`'s own tests and benches, so "the subset passes under the mode" is satisfied by a mode that is a no-op. That is the same defect the `/bin/true` rewrite closed in criterion 6, and it survived here because a criterion phrased as a re-run looks like it inherits the first run's rigour. Two devices, and both must hold:
   * The mode reports how many collections it performed, and the criterion asserts that count is **greater than zero** for the subset. A mode that never collects fails rather than passes.
   * A **negative control**: with one `push_temp` deleted, at least one subset program must fail under the mode. If deleting a root changes nothing, the mode is not observing roots and its pass means nothing. Name the site in the gate report, because a control that is never run is a claim rather than a check.

   Note what this criterion is **not** blocked by. There is one known under-rooted window in `rexx-exec`, `EXIT`'s result from the temps-frame pop through to `exit_code_for`, and it does not obstruct this criterion: nothing in that window calls `alloc_with`, so a faithful collect-on-every-allocation never fires inside it. Several other places imply the opposite, including `Heap::collect`'s own doc, and they are pointing at a real gap for a *future* collector rather than at an obstacle here.

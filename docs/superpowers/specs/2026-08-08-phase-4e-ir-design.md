...
* **Most of the residual is structural, and no in-phase route to the floor exists.** Task 7-M broke the per-promoted-clause cost down with callgrind, removed a quarter of it, and named what remains: a second dispatch level, the call boundary that exists so both engines share one clause wrapper, and values travelling through the register file. Every promotion adds promoted clauses, so the floor moves further away with each task rather than closer.
...
**Two removals of residual that 7-M did not consider, now being measured (2026-08-10, Moritz).** Both are spiked under prototype-publish-revert -- built, measured on both instruments against both arms, reverted without landing -- so the number exists before any redesign is committed to.

* **Boxing the `Raised` variant of `Failure`.** The nine-argument call's return goes through memory because `Failure` is 104 bytes; boxing takes it to 24 and `Result<ClauseRegion, Failure>` to 32. This changes nothing a promotion emits and needs no second implementation of anything, so the `sret` half of the call boundary is **not** inherent and must not be described as such.
* **A shared `enter`/`leave` pair in place of the scoped closure.** "One closure" and "one implementation" are not the same thing. A wrapper split into an enter and a leave, both entered from both engines, keeps exactly one implementation of the clause unit while letting the driver run a region's ops in its own loop. It is not free -- `in_clause`'s epilogue reads the clause's `Result` and `value.rooted()`, and the failure path would have to run the leave from the driver's `?` site -- but harder is not inherent.
...
**The itemisation is not yet delivered, and this criterion is not met until it is.** 7-M's breakdown sums to 80 against its own total of 78, and its largest single row -- 26 instructions of inlined bounds checks and `Result`/`Option` plumbing, a third of the cost -- appears in neither the removed part nor the residual list, which is the same category the removed 19 came out of and is therefore demonstrably reducible. A residual stated as 59 whose numbered bullets reach about 40 is not an itemisation.

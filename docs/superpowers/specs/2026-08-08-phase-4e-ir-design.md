...
**What it buys.** It makes "one engine, two chunks" a visible difference in the op stream rather than a mode flag threaded through every arm; it makes who pays the echo visible instead of an unassertable convention; it makes trace testable where the *oracle differential* cannot test it (`tests/support/mod.rs` normalises at `PREFIX_OFFSET` 7..10 and collapses the space run that carries nesting indent, so that harness cannot see a trace indent -- **but the dual-engine population sweep can, and does: `ir_dual.rs`'s `compare` diffs raw stderr between the two arms, and Task 7's review measured it catching a dropped `>L>` line by name**); an untraced chunk then pays a staleness comparison and an echo branch rather than nothing at all, per D23's amendment; and explicit ops are schedulable and elidable by a Cranelift or wasm backend where side effects in a runtime helper are opaque.
...
Every following line still matched, so only an exact stderr comparison sees it.

**The claim that "no corpus instrument here does" was false, and it was repeated from the spike through six task briefs before anyone ran it (corrected 2026-08-10).** The *oracle differential* cannot see it, because `tests/support/mod.rs` normalises that region. `ir_dual.rs`'s dual-engine population sweep can: its `compare` diffs raw stderr between the two arms, and the normalisation belongs to the differential alone. Task 7's review made the `TraceLiteral` op a no-op and the sweep went red naming `corpus lang/trace_output.rex` and `corpus lang/raise_array_substitution.rex`, and stayed red with Task 7's own case file held out.

**What survives, and it is the part that mattered:** an op that drops a trace line is invisible to the *oracle* comparison, so a promotion that regresses trace and matches the oracle on final output would still pass that harness. What is now known is that the phase built a second instrument that does catch it, which is a stronger position than the spec claimed rather than a weaker one.
...
* **Most of the residual is structural, and no in-phase route to the floor exists.** Task 7-M broke the per-promoted-clause cost down with callgrind, removed a quarter of it, and named what remains: a second dispatch level, the call boundary that exists so both engines share one clause wrapper, and values travelling through the register file. Every promotion adds promoted clauses, so the floor moves further away with each task rather than closer. *(The second half of this bullet is refuted below: an in-phase route exists, was built, and takes two axes under 1.0. The bullet is kept as written because it is what the withdrawal was decided on, and rewriting the input to a decision to match its outcome is how a record stops being one.)*
* **The floor's wall reading was not measurable at the precision it needed.** `emptyloop`'s wall ratio reads 0.9990 at one commit and 1.0228 at the next while executing instruction counts 66 apart in 40 billion -- 40,325,665,756 against 40,325,665,822, a relative difference of 5 in a billion. So a wall reading of this criterion carried at least ±2.3 points of pure code-placement sensitivity, demonstrated with the work held constant, against failures of 2 to 7 points.

**Two corrections to the paragraph above, from Task 7-M's review, recorded because a right decision with a false reason has shipped here three times.**

* **The precision argument is about wall clock only, and does not survive the same commit's decision to stop using wall clock.** On `instructions:u` the measurement reproduces to eight significant figures, so a floor evaluated on instructions and cycles would have been measurable. The withdrawal therefore rests on the structural leg and on the absence of an in-phase route, not on precision. An earlier wording leant on precision as an independent second reason; it is not one.
* **"`in_clause` being a scoped closure is what forces two levels" was asserted and never established, and it is deleted rather than hedged.** What a scoped closure forces is a **call boundary**: the clause's body must sit inside the closure, so the region's ops live in a callee. The second **dispatch level** comes from something else -- a clause's ops being a sub-range of a flat stream -- and a single-level driver with `Clause`/`EndClause` ops would have one match rather than two. 7-M measured only that one particular collapse is worse (`#[inline(always)]` on `run_clause_region`), which does not reach the general claim.

**"The residual is inherent to the two-level shape" is REFUTED BY A BUILD (2026-08-11, Task 7-M2), and the first bullet above is wrong on the half that says no in-phase route exists.** Both removals 7-M declined to consider were spiked under prototype-publish-revert, and one of them clears the withdrawn floor outright:

* **A shared `enter`/`leave` pair in place of the scoped closure removes 152 instructions per `varlookup` pass -- about twice what the whole of Task 7-M removed.** `varlookup`'s IR-minus-tree-walker goes from **+81 to -71 per pass**, taking the axis to **0.98143 on instructions and 0.96988 on cycles**; `alloc4c` goes from +164 to -67, reaching 0.99581. **That is the first time any axis with a promoted clause has passed criterion 4 in this phase, and it passes on both instruments.** `emptyloop`, whose body reaches no promoted clause, pays 24 instructions per pass and goes 1.06596 to 1.08185. **The sharing rule holds by construction rather than by inspection:** `in_clause` and `in_stepped_clause_with` are both *defined in terms of* the enter/leave pair the driver calls, so there is one implementation with two entry shapes -- `enter_stepped_clause`/`leave_stepped_clause` in `run.rs` and `enter_clause`/`leave_clause` in `clause.rs`, each with one body. The failure path predicted to be the hard part does not arise: the clause's `Result` travels into the leave as a value, exactly as `in_clause`'s epilogue already read it.
* **Boxing the `Raised` variant of `Failure` improves absolute speed on both arms and makes the ratio worse on every axis** -- about -70 instructions per pass on both `emptyloop` arms, -121 and -101 on `varlookup`, while `varlookup`'s cycle ratio goes 1.00890 to **1.16911**. So 7-M's "the denominator falls with it" reason is right and its "the promoted path carries one more such return per clause" reason is wrong; the two were stated one sentence apart pointing opposite ways, and the measurement picks. **The `sret` half remains removable either way, so calling it inherent was wrong independently of which direction the ratio moved.**

**What this does and does not settle.** It settles that the residual is not inherent, which is the claim the withdrawal leant on and which this spec asserted. It does not settle whether the enter/leave split should land: it was built to be measured and reverted, not to be reviewed, no patch of it survives, and the trade it makes -- promoted clauses faster, all-`Generic` bodies slower -- turns on the promoted-to-`Generic` ratio, which Tasks 9 and 10 change. **The floor stays withdrawn** because a withdrawn criterion is not a target to re-aim at mid-phase; what changes is that 4f inherits a named, measured route rather than a structural excuse.

**What replaces the guard, because the guard existed for a real reason.** The worry behind the floor was handing 4f a regression it would discover rather than inherit. That is answered by *itemisation* instead of by a threshold: the residual is recorded instruction by instruction, with each remaining removable item named and the reason it was not taken. 4f starts informed.

**The itemisation is now delivered, and 7-M's breakdown was wrong in three separate ways (2026-08-11, Task 7-M2).** The two-instruction discrepancy is in the `stale` row, stated `+7` and measured `+3` -- 7-M's own residual bullet gave that item a range of "4 to 7" and its itemisation took the top of the range. The `-3` row described a codegen redistribution that did not happen; it is `run_ops`' library slice, measuring exactly `-3`. And the `+26` library-helper row **is not a row at all** -- it is the same 78 sliced by source file rather than an independent component, which 7-M's own arithmetic demonstrates. **The three marginals are also not additive:** 16 + 8 + 2 = 26 against the 19 actually removed, so "additive to within one instruction" is false and every "worth N" in that report inherits the error. Change 1 is worth **16**, not the 10 the report used.
...
* **Loop promotion is the largest task and the highest risk.** `run_loop` dispatches five `LoopKind` variants -- it refuses `COUNTER` and `LoopKind::With` before its match -- with `WHILE`/`UNTIL` an orthogonal `Loop.conditional` threaded through it and `run_repeating`, plus the `LEAVE`/`ITERATE` label search and the per-step control-variable re-read a reviewer once caught as a false unreachability claim. `LoopKind::Over` is implemented only in its documented single-iteration form for non-stem targets.
  ...
  > **Correction, 2026-08-15: both things this bullet attributes to `run_loop` now live in `Interp::run_loop_with_header`, and the bullet was accurate when it was written.** The promotion this risk describes is what moved them, so the bullet reads as a description of the code it was assessing rather than of the code that came out. Four dated observations, each read at that commit and none of them a claim about the commits in between: at `47bb1b832` (2026-08-08), which added this bullet, `run_loop` refuses a `COUNTER` and a `LoopKind::With` ahead of its own `match &body.kind`, and that match holds the per-variant arms; at `08137f3ae` (2026-08-10) the match is in `run_loop_with_header` and the refusal is still in `run_loop`; at `b6d548562` (2026-08-10) the refusal is in `run_loop_with_header` too, as one `loop_header_plan(body).is_none()` answer rather than the inline `counter.is_some() || matches!(.., With)` test; and at this tree `run_loop` evaluates the header plan and delegates, holding neither. **`run_loop_with_header` is the function both engines enter** -- the tree-walker through `run_loop`, the compiled stream from `ir::Op::LoopRun` -- so a later phase reading this bullet for where loop dispatch lives wants that one. The risk assessment itself is untouched by the split.
...
* **D23.** The trace setting is an input to compilation, realised as explicit trace instructions in the stream, **plus a per-clause staleness check that makes a stale chunk slow rather than wrong.** Divergent trace output is licensed only from the first optimising pass that is not trace-neutral, priced at Phase 9.
  ...
  **A refinement exists and was not taken, and Moritz's question about call propagation narrows it sharply.** The propagation rules are: `Activation::nested` (a `CALL` to an internal label) **inherits** `trace_mode`; `Activation::routine` (a `::ROUTINE`) resets it to `NORMAL` and does not inherit; and **no callee ever writes it back** -- pinned by `a_callees_trace_setting_does_not_survive_its_return`, where a callee's `trace off` silences only the callee.
  **So a call cannot change the calling body's setting.** The set of bodies that could see a mid-run change is therefore `TRACE` or `INTERPRET` in the body, **not** "or a call" as an earlier version of this note said. That is a much smaller set than the note feared, and it is what would restore "an untraced promoted clause pays nothing" for the great majority of bodies.
  **Moritz also proposed compiling each body to two blocks, traced and untraced.** Half of that is already built: the chunk cache is keyed on the setting, so a body already compiles once per setting it is *entered* under, lazily rather than eagerly. What two blocks would add is a cheap *mid-body* switch -- at a clause boundary, re-point the pc through the other block's `op_of` instead of falling back -- which is on-stack replacement at a boundary the two-level driver already has. It does not remove the need to *detect* the change; the refinement above is what does that. The two compose: the refinement narrows where the check happens, two blocks makes the response cheap.
